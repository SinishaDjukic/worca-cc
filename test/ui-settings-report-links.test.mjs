...
test('the existing About invariants still hold with the new rows', () => {
  ...
  assert.equal(cards.length, 11, 'the new links are ROWS in About, not a twelfth card');
  ...
  assert.equal(view.querySelectorAll('button.info-tip').length, 15, 'still no new ⓘ icon (15 = 14 + Interface mode)');
