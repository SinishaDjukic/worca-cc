...
test('getDb() opens a DatabaseSync at <worcaHome>/worca-cc.db', () => {
  const db = getDb();
...
test('migrate stamps user_version = 15', () => {
  const db = getDb();
  const { user_version } = db.prepare('PRAGMA user_version').get();
  assert.equal(user_version, 15, 'schema version is 15 after migrate');
});
...
test('v8 DB upgrades in place and pre-migration rows survive', async () => {
  await freshHome();
  const db = getDb();
...
test('getDb() calls maybeMigrateFromFs(db) once after migrate()', () => {
  ...
  const { user_version } = db.prepare('PRAGMA user_version').get();
  assert.equal(user_version, 15, 'migrate() ran before the hook');
...
test('getDb() first-launch is concurrency-safe across N processes (no lock/exists crash)', async () => {
  ...
  const db = getDb();
  assert.equal(db.prepare('PRAGMA user_version').get().user_version, 15, 'migrated to v15');
  assert.equal(
