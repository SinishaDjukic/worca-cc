...
/** The newest pipelines row once `pred` holds, polling ≤ 60 s. */
async function waitForRow(pred) {
...
test('wf_auto with humanInLoop:false decides and runs to done; the manifest records the decision', async () => {
  ...
  const row = await waitForRow((x) => x.status === 'done');
...
test('humanInLoop falls back to the project switch, and a question can be answered through /api/answer', async () => {
  ...
  await waitForRow((x) => x.status === 'done');
  ...
  for (let i = 0; i < 1800 && !row; i += 1) {
    ...
    seen = latest;
    if (latest && latest.status === 'stopped') row = latest; else await new Promise((r) => setTimeout(r, 100));
  }
...
test('wf_auto is refused for a workspace target; a saved workflow accepts and ignores humanInLoop', async () => {
  ...
  // The previous test left a STOPPED row, so "running" can only be this run's row.
  const row = await waitForRow((x) => x.status === 'running');
  ...
  await waitForRow((x) => x.status === 'stopped');
});
