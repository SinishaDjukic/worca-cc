...
// schemaGaps() self-heal (a DB stamped 21 by a divergent ladder must still get
...
test('fresh DB: user_version 21, the four ask tables, the index and the §7.1 columns', () => {
  const db = getDb();
  ...
  // ALTER TABLE ADD COLUMN appends, so the v21 column is LAST.
  assert.deepEqual(cols(db, 'ask_run_links'),
    ['thread_id', 'run_id', 'pipeline_id', 'card_id', 'status', 'phase', 'created_at', 'comment_ids']);
});

test('ladder: a v17 DB gets the ask tables and is stamped 21', () => {
...
test('self-heal: a DB already stamped 21 WITHOUT the ask tables gets them from reconcileSchema, stamp untouched', () => {
  const db = new DatabaseSync(':memory:');
  db.exec(MINIMAL_SEED);
...
test('ladder: a v19 DB gets ask_worktrees and is stamped 21', () => {
  const db = new DatabaseSync(':memory:');
  db.exec(MINIMAL_SEED);
  db.exec('PRAGMA user_version = 19');
  migrate(db);
  assert.equal(db.prepare('PRAGMA user_version').get().user_version, 21);
  assert.ok(tableNames(db).includes('ask_worktrees'), 'ask_worktrees created by its own ladder step (current < 20)');
  assert.ok(indexNames(db).includes('idx_ask_worktrees_thread'), 'index created by the ladder');
});

test('self-heal: a stamped-21 DB missing ONLY ask_worktrees is healed, stamp untouched', () => {
  const db = new DatabaseSync(':memory:');
  db.exec(MINIMAL_SEED);
  db.exec('PRAGMA user_version = 17');            // 17 -> 21: run the ask ladder steps
  migrate(db);
