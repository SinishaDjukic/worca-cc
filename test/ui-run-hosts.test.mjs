...
test('every label helper reads the graph; a run with no manifest reads plainly "Running"', async () => {
  ...
  // free text (a usage-limit line); a reasonless pause says who paused it, or plain "Paused" (never "by you").
  ...
  assert.match(np.rdStateCopy({ ...r, status: 'paused', pauseReason: null, lastAction: { kind: 'pause', by: 'ada@example.com' } }, 'Planner'), /^Paused by ada@example\.com\./);
