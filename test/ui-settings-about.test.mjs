...
test('About is the LAST settings card, read-only, with no version baked into the markup', () => {
  ...
  assert.equal(cards.length, 11, 'Appearance + the eight cards (Scheduled runs included), Getting started, then About');
  ...
  assert.equal(view.querySelectorAll('button.info-tip').length, 16, 'About adds no ⓘ icon');
