...
test('About is the LAST settings card, read-only, with no version baked into the markup', () => {
  ...
  assert.equal(cards.length, 8, 'Appearance + the six existing cards plus About');
  ...
  assert.equal(view.querySelectorAll('button.info-tip').length, 13, 'About adds no ⓘ icon');
