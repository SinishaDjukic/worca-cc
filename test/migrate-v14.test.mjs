...
test('a v14-stamped DB missing the additions is healed by the fast-path reconcile, stamp untouched', () => {
  const db = new DatabaseSync(':memory:');
  db.exec(V13_MINIMAL_SEED);
  db.exec('PRAGMA user_version = 14'); // divergent-ladder stamp: version says done, schema says otherwise

  migrate(db); // fast path -> reconcileSchema self-heal

  assert.equal(db.prepare('PRAGMA user_version').get().user_version, 14, 'stamp not rewritten');
  const pipCols = db.prepare('PRAGMA table_info(pipelines)').all().map((c) => c.name);
