...
test('exactly twelve routed views', () => {
  assert.equal((html.match(/data-view/g) || []).length, 12);
});
test('twelve views include composer + the two workspace views + the two agent views + projects + plugins + guardrails', () => {
  for (const v of ['new', 'running', 'history', 'composer', 'workspaces', 'workspace-create', 'agents', 'agent-create', 'projects', 'plugins', 'guardrails', 'settings'])
    assert.ok(html.includes(`data-view="${v}"`), `missing data-view=${v}`);
});
test('nav targets: the base set + workspaces + projects (workspace-create is NOT a nav target)', () => {
  for (const v of ['new', 'running', 'history', 'composer', 'workspaces', 'projects', 'settings'])
