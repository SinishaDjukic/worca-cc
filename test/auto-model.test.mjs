...
const MODELS = [{ id: 'claude-opus-5' }, { id: 'claude-sonnet-5' }, { id: 'claude-sonnet-4-6' }, { id: 'my-proxy' }];

test('resolveAutoModel: env wins verbatim, then the setting (catalog only), then sonnet-5, sonnet, first', () => {
  assert.equal(resolveAutoModel(MODELS, { env: { [AUTO_MODEL_ENV]: ' anything-goes ' }, setting: 'claude-opus-5' }), 'anything-goes');
  assert.equal(resolveAutoModel(MODELS, { env: {}, setting: 'CLAUDE-OPUS-5' }), 'claude-opus-5');
...
test('settings: autoWorkflowModel round-trips through the API, validates BEFORE any write, clears on empty, reports the effective model', async () => {
  ...
  let r = await post({ autoWorkflowModel: 'CLAUDE-OPUS-5' });
  assert.equal(r.status, 200);
  j = await r.json();
  assert.equal(j.autoWorkflowModel, 'claude-opus-5', 'canonical catalog casing');
  assert.deepEqual(j.autoWorkflowModelEffective, { model: 'claude-opus-5', source: 'settings' });
  assert.equal(autoWorkflowModel(), 'claude-opus-5');
  r = await post({ autoWorkflowModel: 'no-such-model' });
  assert.equal(r.status, 400);
  assert.equal(autoWorkflowModel(), 'claude-opus-5', 'a rejected write changes nothing');
  ...
  try {
    ...
    await post({ autoWorkflowModel: 'claude-opus-5' });
  ...
  assert.equal(assertAutoWorkflowModelInput(' claude-OPUS-5 ', [{ id: 'claude-opus-5' }]), 'claude-opus-5');
  assert.throws(() => assertAutoWorkflowModelInput(42), /catalog model id/);
  assert.throws(() => assertAutoWorkflowModelInput('nope', [{ id: 'claude-opus-5' }]), /unknown model "nope"/);
