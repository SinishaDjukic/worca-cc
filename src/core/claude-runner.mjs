...
async function mockAsk({ markers, prompt, cwd, onEvent, signal, resumeSessionId }) {
  ...
  // The team-policy arm, same rule: "policy" plus a verb of intent ("raise the policy cap to $30").
  // It proposes an edit of the context project's per-pipeline cap: `$<n>` in the text, else $30.
  const policy = !wfEvent && !tmEvent && !metrics && /\bpolicy\b/i.test(userText)
    && /\b(?:raise|lower|set|change|edit|make|cap)\b/i.test(userText);
  const workflow = !wfEvent && !tmEvent && !metrics && !policy && /\bworkflow\b/i.test(userText);
  const agents = !wfEvent && !tmEvent && /\bagents?\b/i.test(userText);
  const propose = !wfEvent && !tmEvent && !workflow && !metrics && !policy && /\b(propose|start|run)\b/i.test(userText);
