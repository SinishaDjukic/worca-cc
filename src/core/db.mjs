...
const INCREMENTAL_COLUMNS = {
  pipelines:              { resume_point: 'TEXT', owner_pid: 'INTEGER', owner_host: 'TEXT', heartbeat_at: 'TEXT',
                            ...
                            outcome: 'TEXT',
                            policy_state: 'TEXT' },   // v31: team-policy run state (JSON: home, sha, overrides, exceeded, deviations, reason)
  ...
  workspaces:             { metrics_project: 'TEXT',    // v30: team-metrics home (member absolute path); NULL = no home
                            policy_project: 'TEXT' },   // v31: team-policy home (member absolute path); NULL = no home
};
...
/** v31 (team policy): pipelines.policy_state + workspaces.policy_project — two plain
...
export function migrate(db) {
  ...
  try {
    ...
    if (current < 31) applySchemaV31(db);            // team policy: pipelines.policy_state + workspaces.policy_project
