...
export const MODEL_ERRORS = Object.freeze({
  ...
  secretKey: (where) => `${where} must be a \${VAR} reference to a variable set in worca's environment (or empty to clear) — never the key itself; the user can paste a literal key in Settings › Providers`,
  ...
  signIn: (k) => `${k} is not settable here — the Copilot sign-in and its notice happen on the Providers card (Settings › Providers), by the user`,
