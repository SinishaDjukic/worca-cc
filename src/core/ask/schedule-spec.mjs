 ...
 * @param {{nowMs:number, timeZone?:string, defaults?:{maxFailures?:number}}} opts
 * @returns {{ok:true, schedule:null|object}|{ok:false, errors:string[]}}
 */
export function resolveScheduleSpec(input = {}, { nowMs, timeZone = null, defaults = {} } = {}) {
  ...
  if (when && every) return { ok: false, errors: ['give when (run once) OR every (repeat), not both'] };
  if (when) {
    if (repeatOnly.length) return { ok: false, errors: [`${repeatOnly.join(', ')} only apply with every`] };
 ...
 */
export function createScheduleChangeValidator({ getItem, now = Date.now }) {
  return async function validateScheduleChange(input = {}, { timeZone = null } = {}) {
    ...
    switch (action) {
      case 'run_now': {
        ...
        card.before = kind === 'recurring' ? beforeSeries() : beforeOnce();
      ...
      case 'cancel': {
        ...
        card.before = beforeOnce();
        card.summary = `Cancel "${title}", scheduled for ${card.before.when}`;
        break;
      }
      case 'delete': {
