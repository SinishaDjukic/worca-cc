    ...
    if (!drop.has('cards') && cards.length) {
      ...
      const one = (c) => (c.type === 'workflow'
        ...
        : c.type === 'metrics'
          ...
          : c.type === 'schedule'
            ? `schedule ${label(c.id)} ${label(c.state)} "${clip(c.summary || '', titleMax)}"`
            : `${label(c.id)} ${label(c.state)} (${label(c.workflowId)} on ${clip(c.targetName, titleMax)})${c.schedule ? ` ${clip(c.schedule, 80)}` : ''}`);
