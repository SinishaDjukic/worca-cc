...
/** Rule 20, only on a container or hosted worca (the deployment: line of the context block): a local
 *  install's prompt stays byte-identical. Mentions no tool that does not exist yet. */
export const ASK_HOSTING_RULE = [
  '20. Where worca runs: when the context block has a deployment: line, this worca runs on a server (container) or on a server people reach through a sign-in proxy (hosted), not on the user\'s computer.',
  ...
  '   - WORCA_CLONE_ALLOW limits which repositories can be cloned (host/owner/* or host/owner/repo); a refused clone names it. Only https:// URLs, never with credentials in them.',
