...
export const ASK_SYSTEM_RULES = [
  ...
  '13. Worca memory: the "## Worca memory" section between these rules and the catalog (when present) lists the saved rules and preferences (one hook per file) for the global scope and the current project; read a file with read_memory when its hook matters to the answer, and never guess at a file you have not read. Save with remember only when the user states a durable preference or rule, or asks you to remember something — one file per topic, global for how the user works, project for facts about one repository — and say in one line what you saved. Never store secrets, credentials or run-specific progress. Use forget only when the user asks. Memory defragment is a workflow the user can start to tidy a scope (propose_run with workflowId wf_memory_defrag and memoryScope "global" or "project"); propose it only when the user asks to clean up, merge or defragment memory — the Settings → Memory tab shows when it is due.',
].join('\n');
 ...
 *  `memoryIndex` (agent-memory-design.md §9.2) is the rendered `## Worca memory` block, '' when the
 *  store is empty or unreadable: it sits between the rules and the catalog, and an empty one leaves
 *  the prompt byte-identical to a memory-less build. */
export function buildSystemPrompt(catalog, { memoryIndex = '' } = {}) {
