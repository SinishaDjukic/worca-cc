...
If (and only if) you had to deviate, append a brief, factual note so it survives into the audit. Write/append to `DEVIATIONS.md` in the artifact folder the prompt names for you (under worca that is your step folder; under `/orchestrate` the pipeline directory); if the prompt names no folder, append a clearly marked `## Implementation deviations` section at the bottom of the plan file referenced in the prompt. Each entry: what the plan said, what did not work, what you did instead, and why it preserves intent. Also state deviations in your final assistant note. If you did not deviate, say "No deviations."
