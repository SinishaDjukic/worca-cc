...
**Tech Stack:** Vanilla ES modules, node:test + jsdom 29 (offline), `marked@18.0.10` + `dompurify@3.4.14` served from the P2 vendor routes, the app's existing hljs loader.
...
**v2 (2026-08-23):** both executed dry-runs (D1: Tasks 0-5 deep + 24 mutations; D2: Tasks 0-13 full + 14 mutations) are folded in — every snippet in this revision ran green in a clone at `dbb47f68`; the interval-hang, sanitizer, guardrails-envelope and prefill-clobber defects of v1 are fixed here; counts are measured. Survivor mutations M1/M10/M21/M1b are closed by the four added/extended tests. **v3 (2026-08-23):** the v2 revision was re-executed end-to-end by a zero-context agent — every anchor, snippet, RED shape and count landed exactly, with ONE defect (the meter-cost seed, fixed here); a cold fresh-eyes review's findings (hello-resync scope, elapsed seeding, thread-switch reset, selection-guard on the rebuild path, prefill races, recorded deviations) are folded in. Counts: 3230 → 3360 (the delta-verification pass added the two Task-5 behaviour pins).
...
- **Suite baseline at `dbb47f68`: 3230 pass / 0 fail / 0 skip (~71 s, node v25.6.1)** after `npm ci`. Per-task counts are MEASURED by the executed dry-runs (plus the three survivor-killer tests added in v2, and v3's three: the workspace-Start app-level test and the two Task-5 behaviour pins); the delta-verification pass confirmed the final 3360.
...
- **V3** schedule-on-dirty: one armed `raf(flush)` at a time; `flush` re-arms itself only when the selection guard left dirt. Elapsed: one `setInterval(tickElapsed, 1000)` — elapsed starts on `ask-start` (seeded from its `startedAt` when parseable) and on loading a thread with a turn in flight; it stops on done/error/stop, on `newThread`, and on switching threads.
...
- **V7** card edits live in the DOM only (uncontrolled inputs); `collectCardBody` reads them at Start/prefill time. `update(next)`: still `proposed` → do NOT touch the fields; any other state → renders the terminal stub/link form. Start errors (400/403/409 bodies) render verbatim in `.ask-card-err`, Start re-enabled, fields untouched.
...
- **Send flow:** no thread yet → `POST /api/ask/threads {}` (201) first, store the id + `storeThread`. Then `POST …/messages` with `{text, model, effort, context: getPageContext(), attachments}` → 202 → `model.noteLocalUserMessage(...)` (optimistic; the echo `ask-message` replaces it by id), set the deterministic header title locally from the first text (no frame exists for it), subscribe if this socket hasn't. Elapsed starts on `ask-start` (seeded from its `startedAt` when parseable) and on loading a thread with a turn in flight; it stops on done/error/stop, on `newThread`, and on switching threads. Non-202 → response error verbatim in `.ask-composer-msg` (409 "turn in flight", 429, 400s, 413s).
...
Environmental note (applies to EVERY full-suite run in this plan, Step 13.1 included): a rare PRE-EXISTING teardown race in `test/api-sources.test.mjs` (an `ENOTEMPTY` rmdir inside its `after()`) can red a full run. It is unrelated to P3 — nothing this plan touches is imported there — so re-run the suite, and flag it for a separate hardening ticket rather than "fixing" it here.
...
(The last two tests were added by the delta-verification pass: they pin the ONLY user-visible v3 behaviour changes — elapsed seeding across a reconnect replay, and the send/stop + timer reset on a mid-stream thread switch — and were verified green-with / RED-without their implementation.)
...
Then a SECOND, small Edit in the same step — `loadThread` (written at Task 4, where its text was still correct) must reset the per-turn timer/stop state when a mid-stream thread switch lands. Replace the line
...
Expected: **35 pass / 0 fail** (15 new — measured: the selection test SKIPS at RED (no `.ask-answer` exists yet) and runs its real branch — passing — at GREEN; final suite reports 0 skipped).

- [ ] **Step 5.5: Full suite**

Run: `npm test`
Expected: **3299 pass / 0 fail**.
...
- Send contract (restated): create-thread-on-first-send (`POST /api/ask/threads {}` → 201), then `POST /api/ask/threads/:id/messages {text, model, effort, context, attachments?}` → 202 → optimistic user row by the returned `userMessageId` (the out-of-turn echo replaces it), local deterministic title (`text.split('\n')[0].slice(0, 80)` — first line only, closest to the server's `sanitizeTitle`; NO frame exists for it, and the D13 `ask-title` replacement remains authoritative), clear input+chips, `subscribe`, re-pin. Any non-202 renders the response `error` VERBATIM in `.ask-composer-msg`. Client caps (V9): extension allowlist `.md .markdown .txt .json .csv .log`, dedupe by name newest-wins, max 8, `File.size` > 524288 rejected pre-read, 4 MB thread budget (server bytes + pending). Enter sends, Shift+Enter inserts a newline (composer textarea only).
...
Expected: **45 pass / 0 fail** (10 new; earlier suites stay green — Task 3's shell test still finds `textarea.ask-input`).

- [ ] **Step 6.5: Full suite**

Run: `npm test`
Expected: **3309 pass / 0 fail**.
...
Expected: **35 pass / 0 fail** (10 new; the render and stream suites make no card-shape assertions, so the stub→form change cannot red them).

- [ ] **Step 8.5: Full suite**

Run: `npm test`
Expected: **3329 pass / 0 fail**.
...
Expected: all green — the mount performs no network, adds no markup to index.html, and `askPanel?.` guards every seam. Then `npm test` → **3348 pass / 0 fail**. If ANY previously-green ui-* suite reds here, the failure is almost certainly a boot fetch or an unguarded seam — fix the seam, never the fence.
...
Expected: four tests PASS already — the first three plus the appended workspace-Start test (Task 8 wired the card, Task 10's seams deliver the frames and the `/api/run` POST); the two prefill tests FAIL — `newPipelinePrefill` is never consumed.
...
async function applyAskPrefill() {
  ...
  if (p.target === 'workspace') {
    ...
    // resolves (app.js:5419-5443) — a value written before that rebuild is
...
Expected: **16 pass / 0 fail** (6 card + 10 integration — 6 new).

- [ ] **Step 11.5: Full suite**

Run: `npm test`
Expected: **3354 pass / 0 fail** (fences incl. `ui-newpipeline-*` stay green — `applyAskPrefill` is a no-op when `newPipelinePrefill` is null).
...
**(b) `ui/public/app.js`** — insert the mirrored ask block immediately after the `if (el.budgetReset) { … }` listener block (it ends at ≈`app.js:7514`):
...
and inside `loadSettings()` insert `paintAskSettings(data);` immediately after the `paintBudgetSettings(data);` call (`app.js:7236`, before `paintBudgetReadout();`):
...
- [ ] **Step 13.1** Full suite: `npm test` → expected **3360 pass / 0 fail / 0 skip** (baseline 3230 + 130 new across Tasks 1–12). Green runs exit on their own — if the process hangs, a panel timer leaked (`.unref()`/clearInterval audit, Global Constraints).
...
```
  ...
  - `grep -c "askPanel?.ownsKey(e)" ui/public/app.js` → **2** (both Escape handlers guarded — the two Edits are otherwise mutually indistinguishable to the suite)
  - `grep -c "function buildCard(" ui/public/ask-panel.mjs` → **1** (the tightened stub-name form: the bare name also matches `buildCardTerminal`/`buildCardForm`)
...
Out of scope, deliberately: `docs/storage.md` / `docs/guardrails.md` / `README.md` (§15 assigns docs to the P4 gate), the §12 manual gate (P4), `package.json` (P2 owns the pins), any server change. Two reminders for that P4 gate: the mock `propose` scenario yields a real card only with a real-project page context (P3-F10); and the compact `started` card rendering (recorded in Q&A 10) is on the manual-gate eyeball list.
...
8. **Q: Are the per-task suite counts binding?** — A: Yes as of v2 — every count was MEASURED by the two executed dry-runs at `dbb47f68` (final 3360 = D2's measured 3354 plus the six tests v2/v3 add). A count mismatch during execution is a STOP-and-investigate signal, not a number to adjust silently. The 3230/0 baseline is likewise binding.
