...
> **Status: v3 (refined 2026-08-27). Re-anchored and EMPIRICALLY VERIFIED against `worca/v2-orchestrator` @ `a29adc82` (P1–P4 landed): every shared-core signature, every arithmetic constant and every validator outcome quoted below was executed, not reasoned about. Line numbers in Task 8 are gone — the surgery is anchored on grep-able text, because the v2 line numbers had already drifted 3–11 lines.**
>
> **What v2 got wrong (all fixed here, each was a hard stop):** `autoLayout` returns a `{nodeId:{x,y}}` map, not `{nodes}` — v2's Auto-layout button set `tpl.nodes = undefined`; the shared test `portsFn` had no `verdict`, so the fixture failed V13 forever and Save could never enable; `snap(400+11)` is **407**, not 411; the editor suite imported its fixtures from `ui-graph-view.test.mjs`, re-running all 17 view tests inside it; `graphBounds`/`fitBounds` were used but never imported; the CSS block used 8 of the 10 `--gv-*` vars its own test demands; the shell-order test could not match the shell markup it ships.
...
**Files:** create `test/helpers/graph-view-fixture.mjs`; create `ui/public/graph/view.mjs`; create `test/ui-graph-view.test.mjs`.

> **Why the fixture is a HELPER, not an export of the test file:** `node:test` registers a test the moment its module is evaluated. If `test/ui-composer-editor.test.mjs` imported `fixture`/`portsFn` from `test/ui-graph-view.test.mjs` (v2 did), that import would re-register and re-run all 17 view tests inside the editor process — every "# pass N" in this plan would be wrong and every view test would run twice per suite. `test/helpers/*.mjs` is the repo's existing convention (`test/helpers/graph-ports.mjs`).
...
- [ ] Step 1a: Write the shared fixture helper — `test/helpers/graph-view-fixture.mjs`:
...
//
// VERIFIED 2026-08-27 against the real src/shared/graph modules:
//   validateGraph(fixture(), portsFn)  -> { ok: true, errors: [], warnings: [] }
//   validateGraph(loopFixture(), …)    -> ok as well (review is `md`, matching
//                                         agents/reviewer.meta.json, so V8 is quiet)
// Two details are load-bearing and were BOTH missing in the first draft:
...
and — **back in the RETURNED OBJECT**. The five below are object-literal entries,
siblings of `setStatus`; they are NOT module-private functions and must not be
pasted next to `bandEl` (pasting the two fences as one block is a syntax error):
...
- [ ] Step 3: `node --test test/ui-graph-view.test.mjs` → `Expected: # pass 10 / # fail 0` (Task 1's 5 + this task's 5).
...
- [ ] Step 3: `node --test test/ui-graph-view.test.mjs` → `Expected: # pass 13 / # fail 0` (10 + this task's 3).
...
/* Run-mode executions footer (P6 fills it through view.setFooter, which is the
   ONE writer of a run card's height). These two heights MUST stay var(--gv-foot-h)
   / var(--gv-exec-row-h): nodeSize adds FOOT_H for the first band and EXEC_ROW_H
   for each extra one, and test/ui-graph-css.test.mjs asserts style.css uses the
   FULL --gv-* set injectGeometry writes — dropping them fails that assertion. */
...
.gv-world .xfoot>.fan,.gv-world .xfoot>.xtoggle,.gv-world .xfoot>.xresult{height:var(--gv-foot-h);
...
.gv-world .xfoot .xsq{display:flex;gap:3px;}
.gv-world .xfoot .xq,.gv-world .xfoot .sq{width:7px;height:7px;border-radius:2px;background:var(--line-2);}
...
.gv-world .xfoot .xsum,.gv-world .xfoot .xl{font-size:11.5px;color:var(--ink-2);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
.gv-world .xfoot .xr,.gv-world .xfoot .fl{margin-left:auto;font:10px/1.4 var(--mono);color:var(--ink-3);}
...
- [ ] Step 4: `node --test test/ui-graph-view.test.mjs test/ui-graph-imports.test.mjs` → `Expected: # pass 19 / # fail 0` (17 view + 2 imports).
...
- [ ] Step 1: `npm test 2>&1 | tail -5`. **Expected: BASELINE + 23 new tests** (17 in `test/ui-graph-view.test.mjs` = 5+5+3+4 + 4 in `test/ui-graph-css.test.mjs` + 2 in `test/ui-graph-imports.test.mjs`), 0 failures. Nothing in P5a touches an existing suite: `view.mjs`, `style.css`'s new tail block, `test/helpers/graph-view-fixture.mjs` (a helper — it registers NO tests) and the three new test files are additive.
...
**Files:** delete `ui/public/composer-core.mjs`. Delete these **eleven** test files — `test/composer-ui.test.mjs`, `test/ui-composer.test.mjs`, `test/ui-composer-hint.test.mjs`, `test/ui-composer-legend.test.mjs`, `test/ui-composer-palette-desc.test.mjs`, `test/ui-composer-palette-filter.test.mjs`, `test/ui-composer-steptag.test.mjs`, `test/ui-composer-wires.test.mjs`, `test/connects-to.test.mjs`, **`test/ui-agent-xss.test.mjs`** and **`test/ui-palette-name-clamp.test.mjs`**. Modify `ui/public/app.js`, `ui/public/index.html`, `ui/public/style.css`, `test/ui-workspace-selectors.test.mjs`, `test/ui-agents-view.test.mjs`.

> **The last two are the ones a line-by-line read misses.** `test/ui-agent-xss.test.mjs` (2 tests) drives `#composer-palette .agent-pill` and `#composer-flow`; `test/ui-palette-name-clamp.test.mjs` (3 tests) drives `#composer-palette .pname` / `.pal-chip`. Both go red the moment the v1 markup leaves `index.html`. The command that finds every one of them (run it FIRST, and delete exactly what it prints):
> ```sh
> grep -rln "__composer\\|composer-core\\|EMBEDDED_AGENTS\\|#composer-\\|composer-flow\\|composer-palette\\|composerRefresh" test/
> ```
> The XSS file's origin-trust assertion is NOT lost: Task 5 ports it to `safeAgentIcon` in `test/ui-graph-view.test.mjs`.

> **NO LINE NUMBERS IN THIS TASK.** Every boundary below is a grep-able string, because `app.js` and `style.css` drift between plan-writing and execution (measured on 2026-08-27: the v2 draft's numbers were already 3–11 lines stale, and its `1935-2636` delete would have left the tail of `composerRenderList` behind as a dangling `}` — a syntax error, not a test failure). Resolve each boundary with `grep -n` immediately before you cut.
>
> **Three couplings verified on the branch — get these wrong and the Running/History graphs break:**
> 1. `composerPaintWires` is the SHARED v1 wire renderer: `const paint = (window.__np && window.__np.composerPaintWires) || composerPaintWires;` in `paintRunGraph`, and the `composerPaintWires,` entry in the `window.__np` object (read by `test/ui-run-graph-paint.test.mjs`), both need it. **KEEP it**, together with `COMPOSER_COLORS`, `COMPOSER_TINTS` and `COMPOSER_SEQ` (three consecutive `const`s directly under the composer banner).
> 2. `runNodeAgent` reads `composer.agents[key] || EMBEDDED_AGENTS[key]` for the run-graph icon/label. Both die here, so it gets a real registry cache instead.
> 3. `style.css` declares an UNSCOPED `.node { position:relative; width:230px; … }` (grep `^\.node{position:relative`) that `.run-flow .node` INHERITS — the Run-flow banner says so in its own comment. Re-scope it; do not delete it.

- [ ] Step 1: `app.js` — replace the run-graph's agent-meta source. Delete the `EMBEDDED_AGENTS fallback` comment block that sits directly above `function runNodeAgent(node) {` (grep `EMBEDDED_AGENTS fallback`) and rewrite `runNodeAgent`'s first three lines:
```js
...
function runNodeAgent(node) {
  ...
  return {
```
(the rest of `runNodeAgent` — `icon`/`color`/`label`/`sub` — is unchanged). In `function buildRunGraph(host, manifest) {` add as the first statement: `ensureAgentMeta(() => buildRunGraph(host, manifest));` — one refetch, one repaint, never a loop (the `agentMetaCache.size` guard makes the second call a no-op, and the callback only fires when the fetch actually filled the cache). In `function invalidateAgentCaches()` replace `_composerPaletteDirty = true;` with `agentMetaCache.clear();`.
- [ ] Step 2: `app.js` — the block excision. Resolve the four boundaries FIRST, then cut:
  ```sh
  grep -n "from './composer-core.mjs';"            ui/public/app.js   # LAST line of the import to delete
  grep -n '^// Pipeline Composer module'            ui/public/app.js   # inside the banner that OPENS the block
  grep -n '^function composerRenderList'            ui/public/app.js   # last function of the block
  grep -n '^function modelById'                     ui/public/app.js   # FIRST line that must SURVIVE
  ```
  1. Delete the whole `import { … } from './composer-core.mjs';` statement (it spans ~10 lines; the grep finds its LAST line — walk up to the `import {`).
  2. Delete from the `// ---------------------------------------------------------------------------` banner line immediately ABOVE `// Pipeline Composer module` down to the blank line immediately BEFORE `function modelById`. **Do not compute this range from numbers in a plan** — `composerRenderList`'s closing `}` is the real end, and the draft that shipped hard numbers was already 3 lines short of it, which orphans `  }` + `}` and leaves `app.js` unparseable.
  3. EXCEPT: before cutting, MOVE these two out of the block, to just above the run-graph section (immediately after `runNodeAgent`'s closing `}`), under the banner `// ---- v1 run-graph wire renderer (dies with the v1 engine in P8) ----`:
     - the three consecutive `const COMPOSER_COLORS` / `const COMPOSER_TINTS` / `const COMPOSER_SEQ` lines, and
     - the whole `function composerPaintWires(flowEl, wiresEl, steps, feedbacks, opts) { … }` plus any helper only it calls (check `loopBadge`; if it lives inside the deleted block, move it too).
  4. `modelById` and `option` must SURVIVE — the New-Pipeline rows use them.
  5. Before running any test: `node --check ui/public/app.js` must be silent, and `grep -c 'function composerPaintWires' ui/public/app.js` must print `1`.
- [ ] Step 3: `app.js` — drop the `composer, composerRefresh,` line from the `window.__np = Object.assign(window.__np || {}, {` object; KEEP its `composerPaintWires,` entry. Change `const TIP_SELECTOR = '.info-tip, #composer-palette .agent-pill';` to `const TIP_SELECTOR = '.info-tip, #gv-palette .ap';`. Delete the `if (name === 'composer') initComposer();` line — Task 16 re-adds it. Leave `'composer'` in `VIEW_NAMES` (the 14-entry array; `test/ui-workspace-selectors.test.mjs` asserts it verbatim).
- [ ] Step 4: `index.html` — replace the whole composer view, from the `<!-- ===== VIEW: WORKFLOW COMPOSER ===== -->` comment through the matching `<!-- /view composer -->` comment (grep both; they bracket the section exactly), with the placeholder (Task 9 fills it):
```html
        <!-- ===== VIEW: WORKFLOW COMPOSER (v2 node graph) ===== -->
        <section class="view hidden" data-view="composer"></section>
        <!-- /view composer -->
```
- [ ] Step 5: `style.css` — three surgical edits, all anchored on text (grep each before cutting):
  - **Delete** from the `/* Pipeline Composer — drag/drop canvas, wires, palette, saved list` banner (including the `/* ====` rule lines that wrap it) down to the line immediately BEFORE `.node{position:relative;width:230px;`.
  - **Re-scope, do not delete**, the contiguous run that STARTS at `.node{position:relative;width:230px;` and ENDS at `.node.link-target:hover{…}`: prefix every selector with `.run-flow ` (`.node{…}` → `.run-flow .node{…}`; `.node .nx,.node .loop,.node .selfloop{…}` → `.run-flow .node .nx,.run-flow .node .loop,.run-flow .node .selfloop{…}`), and change the one `.flow .node .nmeta small` selector inside that run to `.run-flow .node .nmeta small`. Add the banner `/* v1 run-graph node look (was the shared composer .node; dies in P8) */` above it. This run is exactly what `.run-flow .node` inherits — deleting it blanks the Running/History graphs.
  - **Delete** from `.builder-foot{` down to and including `.ro-flow .node{width:218px;}`. **KEEP** everything from the `/* Per-agent fan-out toggle in the New Pipeline rows. */` comment onward (`.fanout-toggle`, `.questions-toggle` — New Pipeline still uses them).
  - Do NOT touch the `/* Run-flow — composer-style node graph for Running + History cards` block further down.
- [ ] Step 6: Tests — delete the **eleven** files listed above (re-run the `grep -rln` from the Files note to confirm none is missed). In `test/ui-workspace-selectors.test.mjs` replace the whole `test('composer-core is imported INSIDE app.js via ES6 import (C7), not a separate script tag', …)` block with:
```js
...
```
In `test/ui-agents-view.test.mjs` delete the whole `test('composer save surfaces server warnings via the link-banner toast', …)` block — it drives `#composer-save` / `#composer-link-text` / `window.__composer`, none of which exist any more.
- [ ] Step 7: `node --check ui/public/app.js` (silent), then `npm test 2>&1 | tail -5` → green. **Record the printed total as BASELINE-B2** and the drop from BASELINE-B (that number is the retired suites' test count; do not guess it — copy what the run prints). Also run `grep -rn "composer-core\|EMBEDDED_AGENTS\|__composer\b" ui/ test/ src/` → only the one prose comment in `ui/public/log-line.mjs` ("mirrors composer-core.mjs") may remain; fix anything else.
...
```js
  ...
  // whose own JSDoc says "the caller applies them"). The first draft wrote
  // `tpl.nodes = next.nodes`, i.e. `undefined`: the header's Auto-layout button
  // erased the whole canvas and every later render threw.
...
```js
// AT THE TOP OF THE FILE, beside the existing imports — an ESM import can never
// be nested inside a function:
import { renderNodeInspector, renderWireInspector, renderEmptyInspector } from './inspector.mjs';

// ...and the rest INSIDE createComposer:
  let models = [];
  let efforts = [];
...
- [ ] Step 1: Write the failing test — `test/ui-composer-app.test.mjs`. It boots the real `app.js` under jsdom; the harness is reproduced in full below, because the file it was copied from (`test/ui-composer-wires.test.mjs`) is deleted by Task 8. For a surviving example of the same pattern see `test/ui-agents-view.test.mjs`.
...
- [ ] Step 4: Re-add the router hooks in `app.js`: `if (name === 'composer') initComposer();` in the view-dispatch chain, directly after `if (name === 'projects') loadProjectsView();`, and the leave-guard at the TOP of `showView`, next to the two existing ones (quoted verbatim from the branch — grep `Leave-guard: navigating away from the wizard`):
```js
...
- [ ] Step 2: Implement in `app.js` — add `import { classifyLoops } from '../../src/shared/graph/loops.mjs';`, then branch both builders. **Both already carry a v2 stub** — a three-line comment plus `if (workflow && workflow.version === 2) return [];` at the very top of each function (that stub is exactly why the failing test returns `[]`). REPLACE the `return [];` with the delegating call and drop the stale comment; do not add a second guard above it:
...
- [ ] Step 3: Add the writer next to `async function saveFeedback(workflowId, fbId, maxCycles)`, and route the cycle input to it for v2 workflows (the renderer calls `saveFeedback` today; pass the workflow version through and pick the writer):
```js
...
- [ ] Step 5: Expose the two builders for the tests. Verified on the branch 2026-08-27: `buildNodeConfigRows,` and `buildFeedbackRows,` are ALREADY entries in the `window.__np = Object.assign(window.__np || {}, {` object — this step is a `grep -n 'buildFeedbackRows,' ui/public/app.js` confirmation, not an edit.
...
- **Q8** — Can the v1 composer block be deleted wholesale? → **No. `composerPaintWires` and `COMPOSER_COLORS/TINTS/SEQ` are shared with the v1 RUN graph (`paintRunGraph`'s `window.__np.composerPaintWires` fallback, the `composerPaintWires,` entry in the `window.__np` object, and `test/ui-run-graph-paint.test.mjs`), and the unscoped `.node{position:relative;width:230px;…}` CSS run is inherited by `.run-flow .node`. P5 keeps them (re-homed and re-scoped); P8 kills them with the v1 engine (re-verified on the branch 2026-08-27).**
...
- **Q10** — `PATCH /api/config` body for a loop wire's budget? → **`{ projectDir, workflowId, wires: { [wireId]: { maxCycles } } }`, mirroring today's `feedbacks` writer `saveFeedback(workflowId, fbId, maxCycles)` (rebuild spec §4).**
...
- **Q14** — Why does `hitWireAt` re-sample the wire's cached `d` instead of calling `hitWire(a, b, pt, …)`? → **Because the `d` string is the single source of truth for what is actually ON SCREEN: it already folds in the loop bow and any dangling-endpoint bail-out, and it costs one `getAttribute` instead of two `portAnchor` calls per wire. It is not on the pointer-MOVE path (only `pointerdown`), so the sampling loop is a per-click cost. The one thing it cannot see is the `mirror` flag — which only ever applies to the GHOST, and the ghost is never hit-tested (planner default, xplan §D2).**
- **Q15** — Why is the shared fixture a `test/helpers/` module rather than an export of `test/ui-graph-view.test.mjs`? → **`node:test` registers a test when its module is evaluated, so importing a test file for its fixtures re-runs that whole file inside the importer. `test/helpers/graph-ports.mjs` is the existing precedent (measured 2026-08-27: the alternative silently re-ran 17 view tests inside the editor suite).**
- **Q16** — Why does the test `portsFn` declare a `verdict`? → **V13 rejects any `when: 'blocking'|'clean'` output on a node whose meta carries no `verdict` (`agents/reviewer.meta.json` carries `{"verdict": {"filename": …}}`). Without it `validateGraph(fixture(), portsFn)` returns one permanent error, Save is disabled in every test, and the whole Save/422 half of the plan is untestable (measured 2026-08-27).**
...
## Fixed in v3 (2026-08-27) — every one was verified by executing the real modules

| # | Task | What was wrong | Why it was fatal |
|---|---|---|---|
| 1 | 11 | `autoLayout` returns `{[nodeId]:{x,y}}`, not `{nodes}` | `tpl.nodes = undefined` — Auto-layout erased the canvas |
| 2 | helper | test `portsFn` carried no `verdict` | V13 made the fixture permanently invalid ⇒ Save could never enable |
| 3 | 11 | `snap(400+11)` is **407**, not 411 | the keyboard test asserted an impossible value |
| 4 | 10 | editor suite imported fixtures from `ui-graph-view.test.mjs` | re-ran all 17 view tests inside it; every expected count wrong |
| 5 | 1/3 | `graphBounds`/`fitBounds` used but never imported | `ReferenceError` the moment `fit()` runs |
| 6 | 2 | `setWireBadge` test targeted `w4`, absent from `fixture()` | `TypeError` on a null badge host |
| 7 | 4 | v2 CSS used 8 of the 10 `--gv-*` vars its own test demands, and shipped no footer rules at all | the set-equality assertion could not pass |
| 8 | 4 | the CSS block test sliced to end-of-file | Task 9's `.gv-drag-ghost{border:1.5px…}` tripped the "no geometry number" ban |
| 9 | 9 | shell-order test used `:scope >` + `className.split(' ')[0]` | unsatisfiable by the markup the same task ships |
| 10 | 12 | the "fix" commit left V9 + V20 unresolved | "errors cleared / Save enabled" was unreachable |
| 11 | 8 | hard line ranges, already 3–11 lines stale | the delete orphaned `composerRenderList`'s tail ⇒ `app.js` unparseable |
| 12 | 8 | `ui-agent-xss` + `ui-palette-name-clamp` (5 tests) not in the retire list | suite red after Task 8, with no explanation |
| 13 | 15 | 422 test asserted `isDirty()` on a never-mutated canvas | assertion could not hold |
| 14 | 2 | one code fence merged module-private functions with object-literal entries | pasting it is a syntax error |
| 15 | 1–6 | P5a expected counts (8/11/16/+20) | all understated; now 10/13/19/+23 |

## Known issues (Session A, 2026-08-27 — still open, resolve during execution)
...
- xplan §D2: composer wire hit-testing re-samples the cached `d` instead of calling geometry's `hitWire(a, b, pt, {loop, tol})`. Justified in **Q14** below — but if the ghost's `mirror` flag ever needs to affect hit-testing, switch to `hitWire`.
- xplan §D3: the band/chrome/badge fast paths assume Task 1's `h(tag, cls, text)` helper semantics and per-loop-wire badge hosts — confirmed for `setWireBadge` (a badge host exists only for a wire with an integer `config.maxCycles`; that is now encoded in `loopFixture()`).
- xplan §D7: RESOLVED — see rows 6 and 15 above.
