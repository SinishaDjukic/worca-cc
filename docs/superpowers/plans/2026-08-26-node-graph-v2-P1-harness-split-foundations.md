...
- Every file under `src/shared/**` is pure: only relative imports that stay inside `src/shared`, no `node:` builtins, no `require`, no `process.`, no DOM globals (`window`/`document`/`navigator`/`localStorage` — also never as a bare word in a comment, the guard is a regex), no `fetch(`, no `import.meta`, no top-level `let`/`var`. Enforced by `test/shared-graph-purity.test.mjs` from Task 7 on.
...
- [ ] Step 5: `npm test 2>&1 | tail -5` — record the printed pass count as **BASELINE** (it must be green before you change anything). Do not invent a number; write down what the run prints.
...
**Files:**
- modify `src/core/skills.mjs:111` (`export function collectRequiredSkills(registry, plan) {` — JSDoc directly above at `:101-110`)
...
`Expected: FAIL` — `AssertionError [ERR_ASSERTION]: Expected values to be strictly deep-equal:` with `actual: []` for the Set case (today the function reads `planOrKeys?.steps`, and a Set has none).
...
`Expected: pass` — all three files green (`# fail 0`), including the pre-existing plan-shaped assertions.
...
**This task lands alone and ends green.** It is the only risky task in the plan: ~60 class members and ~2000 of `orchestrator.mjs`'s 4369 lines move file. It is executed by a one-off script (Step 2) so the move is mechanical and reviewable with `--color-moved`, never retyped.
...
> **Deviation from the spec's §5.1 list, deliberate and load-bearing:** the spec files `_phase :3746` under V1-ONLY "(except bookends)". On dev, `_phase` has FOUR call sites — `:515`, `:636`, `:717`, `:1000` — and all four ARE the bookends (per-node phase events come from `_nodeStep :3027`, which stays v1). On top of that, `test/cost-tracking.test.mjs` (13 calls) and `test/duration-tracking.test.mjs` (11 calls) call `orch._phase(phase, cycle, status)` directly and must pass unchanged. So `_phase` moves to the harness with its signature intact, and `_bookend(name, status)` is a thin base method that calls it. No `_phase` remains in `orchestrator.mjs`. P8 deletes `_phase` when those two suites are ported.
...
**The six hooks.** Base versions live at the END of the `RunHarness` class body; the first four throw. `Orchestrator` implements all six.
...
```js
   ...
   *  manifest -> state.stepper (the UI snapshot); agentKeys -> the §9.4 preflight
   *  gate + the skills gate; workflow -> the run's audit line. */
   ...
   *  @param {{resume?:object|null, rehydrated?:object|null}} args resume point + _engineRehydrate's bag
...
`Orchestrator`'s implementations (appended at the end of its class body — this is the v1 code that lives at the ten seams today, moved, not rewritten):
...
```js
  ...
  async _engineRun({ resume = null, rehydrated = null } = {}) {
    ...
    // fills it, on a resume it is rebuilt here. Nothing between the old site
    // (orchestrator.mjs:945 on dev) and dispatch reads this.channelDefs — the only
    // reader is _bindNodeIo, inside _dispatch.
...
plus one new module-level helper in `orchestrator.mjs` (next to `numOr`):
...
**The ten seams** (all inside the moved `run()`/`resume()` shells; `→` = the replacement text). These are the ONLY non-moved lines the review in Step 5 may show.

| # | dev anchor | replaced text (verbatim) | replacement |
|---|---|---|---|
| S1 | `:489`–`:511` | `this.channelDefs = collectChannelDefs(registry); …` through `this.state.stepper = buildStepperManifest(plan, registry);` (incl. the `resolveWorkflow` call `:493-495`, the `if (this.isWorkspace)` fan-out block `:501-507`, the `this._preflightAgentKeys(plan);` line `:510`) | the 8 lines in the S1 block below |
...
| S8 | `:820` | ``if (rp.version !== 1) throw new Error(`resume(): unsupported resume point version ${rp.version}`);`` | `const rehydrated = this._engineRehydrate(rp);` |
...
**Imports after the split** (nothing imported twice, nothing left dangling).
...
`appendAudit` and `today` are imported by BOTH files (both sides call them) — importing the same binding from `./artifacts.mjs` in two modules is not a duplicate import. `collectChannelDefs` splits off from `loadAgentRegistry`/`DEFAULT_AGENTS_DIR` (v1 needs only the first, the harness only the last two). `./protocol.mjs`, `./channels.mjs`, `./runners.mjs`, `./phases.mjs`, `./workflow-validator.mjs`, `./workflows.mjs` and `./recoverable-error.mjs` are NOT imported by the harness at all — that is the point of the split.

**Steps.**

- [ ] Step 1: Write the extraction script to `split-harness.tmp.mjs` at the repo root. It is a ONE-OFF: it reads `src/core/orchestrator.mjs`, locates every member by its exact signature line (never by line number), asserts every expected name is found exactly once, cuts the 63 HARNESS members plus 16 module-level helpers with their leading comment blocks, applies the twelve seam edits (each asserted to match exactly once), and writes both files. **It is embedded in full below.** The `HARNESS_IMPORTS`, `ORCH_IMPORTS`, `BASE_HOOKS` and `V1_HOOKS` constants inside it are the same text as the "hooks" and "imports" sections above — the script's copy is what actually lands; if you change one, change both.
...
const BASE_HOOKS = `
  ...
  /** Run the pipeline to completion or to a pause.
   *  @param {{resume?:object|null, rehydrated?:object|null}} _args
   *  @returns {Promise<'done'|'paused'>} */
  async _engineRun(_args) { throw new Error('engine hook not implemented: _engineRun'); }
  ...
  /** Framework bookend markers ('preflight' | 'done'), emitted by the base for
   *  EVERY engine so both bracket their runs identically. */
  _bookend(name, status) {
    this._phase(name, 0, status);
  }

  /** Constructor seam for the v1 runner registry (v1 only). */
  _initRunners(_opts) { /* base: no runner registry */ }
`;
...
body = once(body, "    if (rp.version !== 1) throw new Error(`resume(): unsupported resume point version ${rp.version}`);",
  `    // Engine hook: rejects a resume point that is not this engine's, and yields
    // the engine-specific fields the shell below rehydrates from.
    const rehydrated = this._engineRehydrate(rp);`);
...
And S12 (kept last because it is written as single-quoted line arrays — the replaced text contains backticks and `${…}`):
...
> Order inside the script: the constants (`HARNESS_HEAD`, `HARNESS_IMPORTS`, `ORCH_IMPORTS`, `BASE_HOOKS`, `V1_HOOKS`) first, then the scan/cut block, then S1–S11, then S12, then the `EXPORTED`/assembly block last.

- [ ] Step 2: `node split-harness.tmp.mjs`
`Expected:` `run-harness.mjs 2693 lines; orchestrator.mjs 1780 lines` (measured 2026-08-27 on a clone of dev `e6968e15`; ±a few lines is fine, an assertion failure is not — every `split: …` message means an anchor moved and you must stop and re-derive it, never loosen the assertion).
- [ ] Step 3: `node --check src/core/run-harness.mjs && node --check src/core/orchestrator.mjs && echo PARSE-OK`
`Expected: PARSE-OK`
- [ ] Step 4: `rm split-harness.tmp.mjs` — it must never be committed. Then `git status --porcelain` must list exactly two entries: `M src/core/orchestrator.mjs` and `?? src/core/run-harness.mjs` (Task 1 is already committed). If `split-harness.tmp.mjs` still shows, you did not delete it.
- [ ] Step 5: Review the move as a move:
`git diff --color-moved=zebra --color-moved-ws=allow-indentation-change -- src/core/orchestrator.mjs`
Everything must render as moved-away (zebra) except: the new import block, `class Orchestrator extends RunHarness {`, the `V1_HOOKS` block, `planAgentKeys`, the `export { isAbort, errStreamAttr };` line, and collapsed blank-line runs. Then read `src/core/run-harness.mjs` and confirm the ONLY non-moved lines are the file header, the import block, the twelve seam replacements (S1–S12 above) and the `BASE_HOOKS` block. Anything else is a bug in the script — fix the script and re-run it from a clean tree (`git checkout src/core/orchestrator.mjs && rm src/core/run-harness.mjs`), never hand-patch the output.
...
`Expected: # fail 0` (measured: 126 + 43 passing across these files with the split applied). The full importer list is `grep -l "orchestrator.mjs" test/*.mjs` — 53 files; the full suite in Step 7 covers them all.
- [ ] Step 7: `npm test 2>&1 | tail -5`
...
**Files:** create `test/run-harness-hooks.test.mjs` (7 tests).

**Interfaces:** consumes `RunHarness` from Task 2; pins the contract P4's `GraphOrchestrator` implements. No production code changes.
...
class StubEngine extends RunHarness {
  ...
  async _engineRun(args) { this.calls.engineRun.push(args); return 'done'; }
...
**Files:** create `src/core/engine-select.mjs`, create `test/engine-select.test.mjs` (5 tests). **No call site changes** — `POST /api/run` (`ui/server.mjs:1062`, factories ≈`:1143`/`≈:1210`), `resumeRun ≈:1560`, CLI run `worca-cc.mjs:1526` and `cmdResume :809` keep calling `createOrchestrator` directly. P4 switches them.

**Interfaces:**
- Produces `selectEngine({templateVersion, resumePointVersion}) → 'v1'|'graph'` (pure) and `async createOrchestratorFor(opts) → orchestrator`.
 ...
 * @returns {Promise<object>} an orchestrator instance
 */
export async function createOrchestratorFor(opts = {}) {
  const resumePointVersion = opts.resume?.resumePoint?.version;
  const templateVersion = resumePointVersion === undefined && opts.workflowId
    ? (await readWorkflow(opts.workflowId))?.version
    : undefined;
  selectEngine({ templateVersion, resumePointVersion });
...
`Expected: # pass 5`, `# fail 0` (measured 2026-08-27).
...
**Files:** create `src/shared/graph/constants.mjs` (the first file of the shared core), create `test/graph-constants.test.mjs` (9 tests).
...
- [ ] Step 1: Write the failing test (full code in Step 3's block below — write the TEST first, watch it fail on the missing module), then implement:
...
- [ ] Step 2: The test — `test/graph-constants.test.mjs`:
...
`Expected` before the module exists: `Cannot find module '…/src/shared/graph/constants.mjs'`. After Step 1: `node --test test/graph-constants.test.mjs` → `# pass 10`, `# fail 0` (9 measured 2026-08-27 + the `BOOKEND_EXECUTION_IDS` pin added by the cross-plan pass).

- [ ] Step 3: Commit — `worca: Node-graph v2 P1 — shared graph constants`
...
`Expected: # fail 0` — 22 tests measured 2026-08-27 (5 new + the two unchanged importer suites).
- [ ] Step 5: `npm test 2>&1 | tail -5`
`Expected:` green at **BASELINE + 28** (Task 1: 2, Task 3: 7, Task 4: 5, Task 5: 9, Task 6: 5). The point of this step is that NOTHING regressed: the total must be BASELINE plus exactly those 28.
...
`Expected: # pass 3`, `# fail 0` (measured 2026-08-27 against the real tree: 23 `ui/public` specifiers, zero false positives).
...
`Expected:` the purity test FAILS with `…/src/shared/graph/mutant.mjs: node: builtin`; after `rm`, re-run → `# pass 3`. (At P1 `ui/public` has no shared imports yet, which is why test 1 exists: without it the walk could pass on an empty tree.)
...
const ROOT = fileURLToPath(new URL('..', import.meta.url));
const SHARED = path.join(ROOT, 'src/shared');
const walk = (d, out = []) => { for (const e of readdirSync(d, { withFileTypes: true })) { const p = path.join(d, e.name); e.isDirectory() ? walk(p, out) : out.push(p); } return out; };
const posix = (p) => p.split(path.sep).join('/');
...
`Expected: # pass 4`, `# fail 0` (measured 2026-08-27; express 4.22.2 → send → mime 1.6.0 serves `.mjs` as `application/javascript`).
- [ ] Step 4: Mutation check: delete the 404-tail `app.use` block and re-run → the 404 test fails with `text/html` (the SPA fallback swallows the miss). Restore it.
...
- [ ] Step 1: If the old branch is available, take it directly and check identity afterwards:
`git show origin/worca-cc/v2-orchestrator-bfb6a0ed:src/core/graph/builtin-workflows.mjs > src/core/graph/builtin-workflows.mjs`
`git show origin/worca-cc/v2-orchestrator-bfb6a0ed:src/core/graph/seed-templates.mjs > src/core/graph/seed-templates.mjs`
then `git show origin/worca-cc/v2-orchestrator-bfb6a0ed:src/core/graph/seed-templates.mjs | diff - src/core/graph/seed-templates.mjs && echo IDENTICAL`
`Expected: IDENTICAL` (and the same for `builtin-workflows.mjs`). If the branch is NOT available, type the files from the blocks below — Task 10's structural tests are the acceptance gate either way.
...
- [ ] Step 3: `src/core/graph/seed-templates.mjs` — header + the first two seeds:
...
**Files:** create `test/graph-seed-templates.test.mjs` (10 tests). No production code changes.

**Interfaces:** consumes `SEED_TEMPLATES/NODE_ID_MAP/FB_WIRE_MAP` (Task 9), `GRAPH_DEFAULT_WORKFLOW/deepFreeze` (Task 9) and `NODE_ID_RE/WIRE_ID_RE/PORT_ID_RE/TEMPLATE_VERSION` (Task 5). The zero-errors/zero-warnings `validateGraph` drift guard against the REAL sidecars is **P2's**, not this task's — P1 has no validator.
...
`Expected: # pass 10`, `# fail 0` (measured 2026-08-27 against the verbatim old-branch files).
- [ ] Step 3: **Mutation audit** (revert each): (a) retarget `wf_full`'s `w13` from `n_check.await` to `n_check.plan` → the await test fails AND the V7 test fails (`n_check.plan` already has `w9`); (b) drop `config` from `wf_full`'s `w12` → the FB_WIRE_MAP test fails ("covers exactly the budget-bearing wires"); (c) change `FB_WIRE_MAP['wf_clarify-implement']` to `{ fb_0: 'w5', fb_1: 'w9' }` → the resolver test still passes (SET equality + per-wire resolve) but the PAIRING pin test fails — that pin is the contract V24's static overlay map (spec §10.2, static maps only) and P3's v1 fixture both follow; (d) add a second wire into `wf_quick-fix`'s `n_review.plan` → the V7 test fails.
...
`Expected:` green at **BASELINE + 45**. The 45 new tests: Task 1 → 2, Task 3 → 7, Task 4 → 5, Task 5 → 9, Task 6 → 5, Task 7 → 3, Task 8 → 4, Task 10 → 10. Reference measurement 2026-08-27 on a clone of dev `e6968e15` with every task of this plan applied except Task 1's two tests: **3803/3803** (so BASELINE 3760 → final 3805 there). If your BASELINE differs, the DELTA is what must be 45.
...
- [ ] Step 3: Hygiene checklist (all must hold):
  ...
  - [ ] No user-visible string changed: `git diff e6968e15 -- ui/public/ index.html | wc -l` → `0`.
...
**Handoff:** P1 is complete. The plan file is `/Users/denislavprinov/Develop/worca-cc/docs/superpowers/plans/2026-08-26-node-graph-v2-P1-harness-split-foundations.md`. Next: **P2 — Shared graph core + sidecars v2 + schema + store** (halves P2a/P2b), which starts by grepping for `export class RunHarness` in `src/core/run-harness.mjs` and `export const SEED_TEMPLATES` in `src/core/graph/seed-templates.mjs`.
...
- **P1-a** — Is the harness move allowed to improve the code it moves? → **No. Byte-identical behavior; the oracle is the 53 orchestrator importers passing unchanged. `git diff --color-moved` must show only the twelve seam edits as non-moved lines** (spec §13 risk 1: "land it alone").
...
- **P1-d** — `_engineRehydrate` returns `{checkpointRef, memberWorktrees, plan?}` per spec §5.1, but the resume shell also writes `Pipeline **resumed** (from <kind> at step <n>).` from v1-only fields. → **The return gains `audit?: string`; the base writes `rehydrated.audit`. v1's bytes are unchanged** (planner default).
...
- **P1-p** — Are the borrowed seed files edited to say V24 instead of V17? → **No. They are byte-identical to `old:src/core/graph/{seed-templates,builtin-workflows}.mjs`; the stale `V17` comments are corrected in P8, where the migration is actually written** (spec §10.2 "copied VERBATIM" + the byte-identity check).
