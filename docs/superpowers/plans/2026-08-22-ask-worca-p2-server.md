# Ask Worca — P2 Server Implementation Plan (v1)
...
**Revision:** v1 (2026-08-22), written after a five-agent recon/adjudication wave (A1 `ui/server.mjs` territory, A2 as-built-vs-contract verification, A3 lifecycle precedents, A4 test conventions + baseline, A5 Fable architecture adjudication). Vendor facts (resolution paths, export shapes, integrities) verified empirically against the exact pins on 2026-08-22.
...
// ── ui/server.mjs seams ──
// _testing gains: askJobs, askFollowers, flipCard, resolveEsmAsset
...
T1/T2/T3 are mutually independent. T4 needs T2 (its tests pin the title call's `permissionMode:'dontAsk'`). T5 needs T4 (`askJobs` entries hold an `AskTurn`). T6 needs T5. T7 needs T3 + T6. T8 last. Execute in numeric order when sequential.
...
Expected: `package.json` `dependencies` gains `"dompurify": "3.4.14"` and `"marked": "18.0.10"` (exact, no caret); `package-lock.json` gains both entries with the integrities above. Nothing else changes in `dependencies`.
...
**Interfaces:**
- Consumes: `runClaude`'s existing `permissionMode` option (already destructured with default `'acceptEdits'`, `claude-runner.mjs:237`).
...
Append to `test/ask-title-options.test.mjs`. The file already imports `test`/`assert` and node fs/path helpers — extend its existing import lines with any specifier below it does not yet have (never add a duplicate import statement). The helper is new and collision-free:
...
(The two tests run the real spawn path — the file's existing `beforeEach` already deletes `WORCA_MOCK`/`ORCH_MOCK`; if it does not, add that `beforeEach` alongside these tests.)
...
and extend the `@param` JSDoc opts line (anchor: the `{{cwd:string, …}}` type at ~`:31`) with `, permissionMode?:string` plus a one-line note:
...
Run: `WORCA_HOME=/tmp/worca-p2-home node --disable-warning=ExperimentalWarning --test test/ask-follow.test.mjs`
Expected: PASS (8/8). Note the guard's `if (detached) return;` first line is what makes the post-detach question test pass even though `error`'s own handler called `detach()` while iterating.

- [ ] **Step 5: Full suite**

Run: `npm test`
Expected: **3181 pass / 0 fail** (3173 + 8).
...
test('happy path: frames ordered, session stored immediately, row + totals persisted before ask-done', async () => {
  ...
  const { turn, frames } = makeTurn(s, {}, {
    runClaudeImpl: async (opts) => {
      ...
      assert.equal(getThread(s.thread.id).sessionId, 'sess-1', 'session id stored the moment it arrives');
...
test('R-A: valid proposal → card persisted mid-turn and ask-card precedes ask-done', async () => {
  ...
  const { turn, frames } = makeTurn(s, {}, {
    validateProposal: async (input, { cardId }) => {
      assert.match(cardId, /^card_[0-9a-f]{8}$/);
      assert.equal(input.brief, 'do it');
      return { ok: true, card };
    },
    runClaudeImpl: async (opts) => {
      ...
      await new Promise((r) => setImmediate(r));
      const mid = getMessage(s.asst.id);
      assert.ok((mid.blocks || []).some((b) => b.kind === 'card' && b.state === 'proposed'),
...
test('R-A settle race: a hook still pending when the user stops does not hang the turn', async () => {
  ...
  const { turn } = makeTurn(s, {}, {
    ...
    runClaudeImpl: async (opts) => {
      ...
      await new Promise((r) => { opts.signal.addEventListener('abort', r, { once: true }); });
      throw err;
    },
  });
  const done = turn.run();
  setImmediate(() => turn.stop());
  const out = await done;                     // settle() raced against the abort — resolves
...
class AskTurn extends EventEmitter {
  constructor({
    ...
    this.deps = {
      ...
      // (orchestrator.mjs:2630 _backoff precedent). Tests inject both.
   ...
   * costOverride null forces cost_usd NULL (spec §6.2.8 — no `result` arrived).
   */
  async _complete({ kind, status, reason = null, message = null, errorClass = undefined, costOverride }) {
    ...
    const costUsd = costOverride === null ? null : summary.costUsd;
  ...
  async run() {
    ...
    try {
      ...
      // One 15-minute budget for the whole turn, retry included. Flag BEFORE
      // abort so R-C can tell a timeout from a user stop (plugin-shim.mjs:164).
    ...
    for (let attempt = 1; attempt <= 2; attempt += 1) {
      ...
      try {
        ...
        if (err?.name === 'AbortError') {
          const costOverride = s.sawResult ? undefined : null;
          if (this.timedOut) {
            return await this._complete({ kind: 'error', message: 'timed out after 15 min', costOverride });
          }
          return await this._complete({ kind: 'done', status: 'stopped', reason: 'user', costOverride });
        }
...
Run: `WORCA_HOME=/tmp/worca-p2-home node --disable-warning=ExperimentalWarning --test test/ask-turn.test.mjs`
Expected: PASS (15/15). If the mid-turn card assertion flakes, the two `setImmediate` awaits in that fake impl are the knob — `_onProposal` resolves on the microtask queue after the validateProposal promise, so two macrotask turns are enough deterministically.

- [ ] **Step 5: Full suite**

Run: `npm test`
Expected: **3196 pass / 0 fail** (3181 + 15).
...
Expected: FAIL — every `/api/ask/*` fetch returns the SPA shell/404 (routes missing), `hello.ask` is `undefined`, `summary.ask` is `undefined`.

- [ ] **Step 3: Implement the server hunks**

**Hunk A — imports.** Below the existing `src/core/settings.mjs` import block (~`:22-37`) add (merge with any existing import from the same module — never duplicate an import statement; `listProjects`, `readWorkspace` and the artifacts readers are already imported elsewhere in the file):
...
import {
  ...
  setThreadTitle as askSetThreadTitle,
} from '../src/core/ask/store.mjs';
...
**Hunk B — registry + helpers.** New banner section directly after the guardrails routes end (~`:2820`), before `/api/agents*`:
...
/** hello payload: running turns only (§8.2). */
function askHello() {
  const out = [];
  for (const [threadId, job] of askJobs.entries()) {
    if (job.status === 'running') out.push({ threadId, messageId: job.messageId });
  }
  return out;
}
...
1. Query parse (~`:222-232`): add `requestedThreadId` next to the other three:
```js
...
app.get('/api/ask/threads/:id', (req, res) => {
  ...
  try {
    ...
    res.json({
      ...
      inFlight: job ? { messageId: job.messageId } : null,
    });
  } catch (err) {
    res.status(500).json({ error: err && err.message ? err.message : String(err) });
  }
...
Run: `npm test`
Expected: **3206 pass / 0 fail** (3196 + 10). Watch the fences: `agentgen-api` (hello shape gained a key — its tests read `msg.runs`, not the whole frame, so they stay green), `scan-api`, `ui-server-stepper-seed`.
...
const framesFor = (msgs, threadId) => msgs.filter((m) => m.threadId === threadId && typeof m.type === 'string' && m.type.startsWith('ask-'));
...
test('a full mock turn: 202, stamped frames to ask-done, persistence, session, title, echo', async () => {
  ...
  assert.equal(snap.thread.title, 'hi there', 'deterministic title stamped on the first message');
  assert.equal(snap.inFlight, null);
  ws.close();
});
...
Expected: FAIL — the message POST 404s (route missing).

- [ ] **Step 3: Implement — context resolver, mock card, the two routes**

Add inside the ask section (after the Task 5 helpers):
...
async function resolveAskContext(threadId, ctx = {}, listedAttachments = []) {
    ...
    if (ctx.workspaceId) {
      ...
      if (ws) {
        out.workspace = {
          ...
          members: (ws.projects || []).map((m) => m.projectName || m.name).filter(Boolean),
        };
      }
    }
  } catch { /* absent line */ }
  ...
  try {
    ...
    const listedIds = new Set(listedAttachments.map((a) => a.id));
...
app.post('/api/ask/threads/:id/messages', async (req, res) => {
  ...
  try {
    ...
    if (thread.title == null) {
      ...
      askSetThreadTitle(id, deterministicTitle); // §7.4 — no frame for the deterministic title
    }
    ...
    const asstMsg = askAppendMessage(id, { role: 'assistant', text: '', status: 'streaming', model: mv.model, effort: mv.effort });
    ...
    const headerCtx = await resolveAskContext(id, cv.context, listed);
    ...
    // Register the job (atomic replace of a grace entry — §8.3) and start.
    const prev = askJobs.get(id);
    if (prev && prev.graceTimer) clearTimeout(prev.graceTimer);
    const job = {
      turn: null, messageId: asstMsg.id, userMessageId: userMsg.id,
    ...
    res.status(202).json({ userMessageId: userMsg.id, assistantMessageId: asstMsg.id });
  } catch (err) {
    res.status(500).json({ error: err && err.message ? err.message : String(err) });
  }
});
...
Run: `WORCA_HOME=/tmp/worca-p2-home node --disable-warning=ExperimentalWarning --test test/ask-api-messages.test.mjs`
Expected: PASS (9/9). The `MOCK_SLOW` tests take ~2-5 s each (300 ms × ~8-15 frames).

- [ ] **Step 5: Full suite**

Run: `npm test`
Expected: **3215 pass / 0 fail** (3206 + 9).
...
let cwdSandbox = null;
let homeDir, srv, base, wsBase, mod, prevHome;
let projectDir, projectKey, workspaceId;
const JSONH = { 'Content-Type': 'application/json' };
const MODEL = { model: 'claude-opus-5', effort: 'high' };
...
before(async () => {
  ...
  // A registered project (the mock card's target) + a one-member workspace.
  ...
  const wsRes = await fetch(`${base}/api/workspaces`, {
    ...
    body: JSON.stringify({ name: 'Team', projects: [projectDir] }),
  });
...
test('project card: propose → Start via /api/run → started flip, notice, follower done + run-status', async () => {
  ...
  assert.ok(['done', 'error'].includes(snap2.runLinks[0].status) || snap2.runLinks[0].status === 'done');
  w.ws.close();
});
...
test('/api/run pair validation: 400 on half a pair, unknown thread, unknown card; 409 on non-proposed', async () => {
  const body = { projectDir, prompt: 'x', workflowId: 'wf_default' };
  assert.equal((await post('/api/run', { ...body, askThreadId: 'ask_ffffffff' })).status, 400);
  assert.equal((await post('/api/run', { ...body, askCardId: 'card_ffffffff' })).status, 400);
  assert.equal((await post('/api/run', { ...body, askThreadId: 'ask_ffffffff', askCardId: 'card_ffffffff' })).status, 400);
...
test('dismiss: {block} on success, 400 on other states, 404 unknown, 409 when not proposed', async () => {
...
Expected: FAIL — the dismiss route 404s through the SPA fallback, `/api/run` ignores the ask fields (no flip, no link row).
...
**Hunk B — `/api/run` validation.** Immediately after the workspaceId/projectDir mutual-exclusion checks (anchor: the two `badRequest` returns at ~`:792-796`), BEFORE the source/prompt handling and the budget gate — a 403 must not leave a half-linked card, and per spec §8.1 every ask-field failure is decided "before the run is created":
...
**Hunk D — `_testing`:** append `flipCard` to the export object.

- [ ] **Step 4: Run to verify green**

Run: `WORCA_HOME=/tmp/worca-p2-home node --disable-warning=ExperimentalWarning --test test/ask-api-cards.test.mjs`
Expected: PASS (7/7). Slowest file of the three (mock pipelines + MOCK_SLOW): expect 15-40 s.

- [ ] **Step 5: Full suite**

Run: `npm test`
Expected: **3222 pass / 0 fail** (3215 + 7). `/api/run` fences: `workspaces-api`, `api-workflows`, `api-workflow-defaults`, `ui-server-stepper-seed`, `ui-server-question-replay` all untouched-green (the ask hunks are no-ops without the ask fields).
...
Expected: **3222 pass / 0 fail** both times (flake shield). If `test/api-sources.test.mjs` ENOTEMPTYs or the skills-gate timing flake fires, re-run the single file before investigating.
...
**Known soft spots the review wave must execute (not design risks):** (1) `resolveAskContext` reads pipeline-row fields (`title`/`status`/`startedAt`/`branch`) defensively — verify the exact lite-row field names against `artifacts.mjs` during the dry run (header-only impact); (2) `createThread({title})` signature was AVOIDED (create + `updateThread`) because P1's signature is unverified; (3) `listAttachments` ordering for the header's "earlier attachments" slice is assumed oldest→newest (`.slice(-5)`) — labels only; (4) the `Run finished — "title" · done · 3m12s · $0.42` wording is this plan's contract (spec §9.5 wants "status / duration / cost / link" without exact copy) — P3 renders `notice.text` verbatim, so the dry run must not "fix" the wording without updating T3's tests.
...
4. **Counts:** 3166 → T1 3171 → T2 3173 → T3 3181 → T4 3196 → T5 3206 → T6 3215 → T7 3222.
