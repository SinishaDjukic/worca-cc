# Ask Worca Cost in Statistics — Implementation Plan v2

> v2 of 2026-08-23-ask-cost-statistics.md — refined per 5-agent review (2 executed TDD dry-runs with mutation audits, anchor fact-check, spec cross-check, adversarial critique), 2026-08-23. 10 mutation survivors from v1's test suite are killed by the added/strengthened tests.
...
- Test runner: `npm test` = `rm -rf .worca-cc-test && WORCA_HOME=.worca-cc-test node --test test/*.mjs`. Single file: `rm -rf .worca-cc-test && WORCA_HOME=.worca-cc-test node --test test/<file>.mjs`. In a fresh worktree run `npm ci` first (skipping it produces bogus express failures). Baseline suite size at plan time: 3369 (measured 2026-08-23 at 1074c7d8) — always trust the Task 0 measured figure over this number.
...
Expected: all green. Record the pass count (last known baseline 3369; any failure here is pre-existing — stop and report, do not fix).
...
**Files:**
- Modify: `src/core/db.mjs` (`:54` SCHEMA_VERSION; new DDL const after `MODEL_COST_FLAGS_DDL` (`:545-550`); `schemaGaps` `:641-673`; `repairSchemaGaps` `:677-686`; `reconcileSchema` `:699-711`; new `applySchemaV19` after `applySchemaV16` `:803`; ladder `:854-855`)
- Create: `test/migrate-v19.test.mjs`
- Modify: `test/db.test.mjs:137-146`, `test/ask-db-schema.test.mjs:25,41,48,50,62`, `test/db-pause-schema.test.mjs` and every other `user_version` 18-pin (list in Step 5)
...
Expected: FAIL — the first test's fresh-install pin fails (`ask_cost_ledger` absent from a fresh DB); later tests fail on `user_version` 18 / missing table.
...
- `test/db.test.mjs:137-141` (test name string + assert), `:146`, `:266` (`'migrate() ran before the hook'` assert), `:316` (assert + its `'migrated to v18'` message string → v19)
- `test/db-pause-schema.test.mjs:23` — the pin is SPLIT across two lines (`const v = …user_version;` at `:22`, then `assert.equal(v, 18);` at `:23`): bump the `18` in the assert. This file is easy to miss — no `user_version` on the assert line itself.
...
```bash
grep -rn -A1 "user_version" test/*.mjs | grep -w 18 | grep -v migrate-v19
```

Expected: no output. The -A1 catches pins split across two lines (db-pause-schema's aliased assert). The v19 test's own deliberate PRAGMA user_version = 18 rewind lines are excluded by the migrate-v19 filter — they stamp the PRE-v19 version on purpose. Do NOT touch: test/db.test.mjs:120/:126 (the '18 spec tables' name and EXPECTED_TABLES.length — that list has never included ledger/config tables), historical stamps (= 15, = 17, …), test/ask-db-schema.test.mjs:39 (= 17 setup).

Also bump the stale prose so names stop lying (no assertions involved): test/ask-db-schema.test.mjs:2 (bump ONLY the 'stamped 18 by a divergent ladder' phrase — line 1's 'the v18 ladder step' names the step itself and stays 18), :23 ('user_version 18' in the test name), :36 ('…is stamped 18'), :45 ('already stamped 18'); test/migrate-v15.test.mjs:27 ('lands on user_version 18'), :58 ('version stays 18').

- [ ] **Step 6: Run the schema-affected suites**

Run: `rm -rf .worca-cc-test && WORCA_HOME=.worca-cc-test node --test test/db.test.mjs test/db-pause-schema.test.mjs test/ask-db-schema.test.mjs test/migrate-v1*.test.mjs test/subagent-migration*.test.mjs test/upgrade-integration.test.mjs`
Expected: PASS.
...
In `test/cost-budget.test.mjs`, extend the cost-budget import (`:9-12`) with `recordAskCostDelta, askWindowedSpendUsd, totalWindowSpendUsd` — only these three are new (`allTimeTotals`, `roundUsd`, `costWindowStart`, `recordCostDelta`, `budgetStatus`, `getDb` are already imported; adding `allTimeTotals` again is a duplicate-binding SyntaxError); `setTotalCostLimitUsd`/`setCostLimitResetPeriod` are already imported from settings at `:13-14`. Append:
...
(The file's own cleanup convention is `setCostLimitResetPeriod('')` — see the budgetStatus test's closing lines at `:125-126`.)
...
Expected: FAIL — both files die at module load with `SyntaxError: The requested module '../src/core/cost-budget.mjs' does not provide an export named '…'`, naming one of the newly-added bindings (which one Node picks is version-dependent — any of the three new names counts as the expected red; the red phase carries no per-assertion signal — that arrives after Step 3).
...
Note: budgetStatus() has consumers beyond the orchestrator — /api/budget (ui/server.mjs:2336), the run-start (:944) and resume (:1394) gates, three CLI call sites (src/cli/worca-cc.mjs:684/:749/:1501) and the sidebar/readout/banner renderers. All are total-budget surfaces D3 wants combined; they pick the new figure up automatically. No edits.
...
- Modify: `test/ask-store.test.mjs` (Step 5 sweep + ledger pin)
- Test: `test/ask-turn.test.mjs`, `test/ask-store.test.mjs`

**Interfaces:**
- Consumes: `recordAskCostDelta` (Task 2), `summary = this.reducer.finish()` (`{usage:{input,output,cacheRead,cacheCreation,ctx}, costUsd, …}` — `events.mjs:411-433`; `ctx` is a context-fill snapshot and must NEVER be summed into `tokens`), `this.model`, `d.now()`.
- Produces: dep `recordAskCost` on `AskTurn.deps` (default `recordAskCostDelta`, injectable in tests); one ledger row per costed turn.

- [ ] **Step 1: Write the failing tests**

In `test/ask-turn.test.mjs`: add to the imports `getDb` from `../src/core/db.mjs` (new import line). Add near the helpers:

```js
const clearAskLedger = () => getDb().exec('DELETE FROM ask_cost_ledger');
```

`deleteThread` is already imported (`:11-14`). Append six tests (the file's fakes: `say`/`push`/`RESULT` at `:18-26` — `RESULT(over)` accepts a usage override; stop tests use `setImmediate(() => { turn.stop(); })`):
...
Expected: five of the six new tests FAIL (row/call count 0 ≠ 1 — the spy test's `calls.length` is 0 too: the unknown `recordAskCost` dep is ignored by the pre-change deps object); 'a turn that ends before a result…' is a negative assertion and is vacuously GREEN before the writer exists — do not hunt for its failure; every pre-existing test still passes.
...
- [ ] **Step 5: Pin the sweep (spec §9)** — in `test/ask-store.test.mjs`, extend the existing `sweepEmptyThreads` test: after its existing sweep assertions, seed one more old empty thread, give it a ledger row, and assert the sweep still deletes the thread but never the ledger row:
...
Anchor: the test `'sweepEmptyThreads removes only message-less threads older than the cutoff'` (`test/ask-store.test.mjs:349-360`) — insert immediately after its final line `assert.ok(getThread(newEmpty.id));`. The surrounding test already defines `const old = '2000-01-01T00:00:00.000Z';` and imports `createThread`/`getDb`/`sweepEmptyThreads`, so the snippet pastes verbatim. Re-run test/ask-store.test.mjs.

- [ ] **Step 6: Commit**

```bash
git add src/core/ask/turn.mjs test/ask-turn.test.mjs test/ask-store.test.mjs
...
…then append directly after the five `recordAskCostDelta` lines, still inside `seedWorld()`:
...
(`seedPipeline`/`setStarted`/`iso` are the file's own helpers. 2026-09-15 sits after range=all's `windowEnd` of 2026-09-01, so `cohortTotals(0, windowEnd)` skips it — `all.totals.runs` stays 5 and `series.length` stays 6 — while `allTimeTotals()` counts it: 1 + 2 + 5 = 8. Every other test in the file passes with the extra row; confirm in the red run.)
...
(no roundUsd import needed — the expected figures are literals).

- [ ] **Step 2: Run to verify failure**

Run: `rm -rf .worca-cc-test && WORCA_HOME=.worca-cc-test node --test test/stats-api.test.mjs`
Expected: FAIL — range=week red first ("0.75 !== 1.15", the combined-spend assert), range=all red on `totals.ask` undefined.
...
- `:28` test-name string `'renderKpiRow: 4 tiles, …'` → `'renderKpiRow: 6 tiles, …'` (the name must stop lying).
...
**Files:**
- Modify: `ui/public/app.js:576-579` (ask-frame WS branch)
- Modify: `test/ui-stats.test.mjs` (`STATS_FIXTURE` `:18-32`; new test)
- Modify: `test/ui-budget-indicator.test.mjs` (new test)
...
(Foreign threadId — askPanel.pushServerFrame early-returns on it, ask-panel.mjs:1433; the stubs are safe.)

1c. In the existing navigation test that asserts `#stats-body .stat-row` after `showStats()`, add the D4 smoke: `assert.equal(window.document.querySelectorAll('#stats-body .stat-tile').length, 6);` — the one test that boots the REAL app.js + renderStatsBody against a payload-shaped fixture.

1d. In `test/ui-budget-indicator.test.mjs`, append (harness: `ctx.counts.budget` + `ctx.wsBox`, same pattern as the run-finishing test):
...
(`ctx.counts.budget`, `ctx.wsBox.ws.dispatch`, and `ctx.tick()` are the file's real helper names — same shape as the run-finishing budget test at `:190-205`; the snippet pastes verbatim at end-of-file, which the file's `:277-280` leaked-tick note makes safe.)
...
Frames reach every open tab — stampAskFrames ends in a global broadcast (ui/server.mjs:3025-3031) — so a second window with Statistics open tracks chat spend live, exactly D12's intent.
...
```bash
git add ui/public/app.js test/ui-stats.test.mjs test/ui-budget-indicator.test.mjs docs/storage.md
...
Expected: green, pass count = Task 0 baseline + 20 new tests (Task 1: +5, Task 2: +4, Task 3: +6, Task 4: +0 files (edits), Task 5: +3, Task 6: +2). At the 3369 plan-time baseline that is exactly 3389 — the figure the plan's verification run measured; trust the Task 0 measured baseline if it moved.
...
- Spec D1–D12 → tasks: D1/D9 (Task 1), D2/D10/D11 (Tasks 2–3), D3 (Tasks 2 + 4 — budget/enforcement in 2, stats totals/series in 4; orchestrator gate at orchestrator.mjs:2543, found by the caller grep the spec §7.3 mandated), D4–D7 (Task 5), D8 (no code — Spent tile reads combined figures automatically), D12 (Task 6), §7.5 API (Task 4), §10 docs (Task 6), §11 no-flag rollout (Task 1 backfill).
...
- Spec §9's ask-store row is covered by Task 3 Step 5 (sweep + ledger pin); spec §9's db.test.mjs row (EXPECTED_TABLES 18→19) is wrong — that list has never included ledger/config tables, and the check is subset-based.
- Spec §10 says "add ask_cost_ledger to the schema/table list" in docs/storage.md — that file has no table list; folding the note into the ask/<threadId>/att layout entry (Task 6 Step 5) is the only workable reading.
- Adjudicated no-changes: Date.parse(created_at) is safe (store.mjs:33 writes ISO-Z, sole writer); ledger ts = completion time (parity with the pipeline ledger); no boot self-heal for the downgrade hole (cost_ledger precedent — documented in db.mjs + turn.mjs comments instead); round-of-rounds in totalWindowSpendUsd is the spec formula and matches stats' structure; no debounce on loadStatsView (one frame per turn, ≤3 concurrent turns).
