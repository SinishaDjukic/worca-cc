# Ask Worca — P1 Core & Runner Implementation Plan (v2)
...
**Revision:** v2 (2026-08-22) — v1 reviewed by four agents (anchor check, two executed TDD dry-runs incl. the real fixture capture, Fable adjudication); every finding folded in; see "Self-review" item 5.
...
- **R-C — classifying a runner rejection.** `const s = reducer.snapshot()`: if `/max_turns|max_budget/.test(s.resultSubtype ?? '')` → `ask-done{status:'stopped', reason: s.reason}` (probe F5: the CLI exits 1 on those); else if `resumeSessionId && (!s.sawInit || s.errors.some((e) => /No conversation found/.test(e)))` → the §6.2.7 retry without `--resume` (the narrower predicate avoids clearing a healthy `session_id` on an unrelated auth/network failure where `sawAssistant` is also false); else `ask-error{message}`. In every branch call `finish()` exactly once, after `settle()`.
...
| `ui/server.mjs` | modify (4 hunks: `:30-36` import, `:2159-2165`, `:2175-2210`) | settings import, `settingsState()` keys, POST normalisation/validation/setters, legacy root guard (T5) |
...
and add, after that test's closing `});` (`:322`), the regression the spec requires (pattern: `test/budget-api.test.mjs:90-99`) — the multi-key 400 proves the SET validation runs before any write:
...
test('list_runs: limit is clamped to 1..100, default 20 (25-row fake)', async () => {
  const many = Array.from({ length: 25 }, (_, i) => ({ ...LITE[1], id: `r${String(i).padStart(7, '0')}`, title: `Run ${i}`, mtime: 1000 - i }));
  const t = createAskTools({ ...fake, listAllPipelines: async () => many });
  assert.equal((await t.call('list_runs', {})).length, 20, 'default 20');
  assert.equal((await t.call('list_runs', { limit: 1000 })).length, 25, 'clamped to 100 — all 25 rows');
  ...
  assert.equal((await t.call('list_runs', { query: 'run 2' })).map((r) => r.title).length, 6, 'substring match: Run 2, Run 20..24');
...
- [ ] **Step 3b: Export `mockEnabled`** — change `function mockEnabled(opts) {` (`claude-runner.mjs:188`) to `export function mockEnabled(opts) {`. P2 uses it to decide whether a turn runs in mock mode (`turn.mock = {card}`); nothing else changes.
...
Expected: seven `captured <name>: …` lines, the table, `total cost ≈ $0.07` (measured 2026-08-22: $0.0655 — the header's ≈ $0.15 is an upper bound), and `test/fixtures/ask/` containing 14 files. If `task-subagent` fails with "did not finish in the FOREGROUND shape", `CLAUDE_CODE_DISABLE_BACKGROUND_TASKS` did not reach `claude` — check `buildAskSpawnOptions().modelEnv` and `prepareModelEnv` (`model-env.mjs`) before re-running. If the capture aborts on a recipe violation, do NOT hand-edit fixtures — fix the recipe and re-run.
...
Run: `npm test 2>&1 | tail -5`
Expected: `# fail 0`; the count grows by the 19 new files (≈ 110 new tests; the dry runs measured 3005 → 3070). `git status` shows nothing untracked except `docs/superpowers/` (never committed) and the usual `marketing/`.
