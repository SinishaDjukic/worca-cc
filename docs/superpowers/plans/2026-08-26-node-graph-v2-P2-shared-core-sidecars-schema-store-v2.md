...
> **v2 (refined 2026-08-27, Session A):** v1 + cross-plan contract alignment (xplan A1/A10/A16/A19/A20/A30/A31) + Opus anchor fact-check (86 OK/20 DRIFT/7 WRONG) + executed dry-run in a clone (P1 applied first; see counts) + Fable critique (F1–F24 applied). Wave 2 (2026-08-27): cold Fable fresh-eyes review applied (G1 — backticks inside the `writeGraphWorkflow` SQL template literal, a `SyntaxError` — plus G2 test filenames, G3 `patch` TDZ anchor, G4–G11 minors); the full re-execution from a reset clone (P1 v2 applied first): ALL 26 tasks green, every gate exact — 3760 → 3810 (P1 v2) → **3907** (P2a, BASELINE + 97) → **3949** (P2b, BASELINE-B + 42) — 27/29 mutations red; the two survivors (the B9 `app.js` guards needed a fixture with stale v1 cells; V5's `toId` guard needed an inbound wire) are fixed in this text, plus seven small execution findings (first-missing-binding `Expected` texts, `writeGraphWorkflow` placement anchor, empty half-marker commits, the `:3928` grep comment, the +15 `ui/server.mjs` anchor note). Counts marked "re-measured" are confirmed by that run. Anchors valid for dev @ e6968e15.
...
# SHAPE probe — P1 exports KINDS/PORT_TYPES/FLOW_KINDS as frozen ARRAYS, not Sets.
# Every membership test against them is `.includes(...)`; a Set-style `.has(...)`
# throws `TypeError: <name> is not a function` on the FIRST validateGraph call
# and takes all 23 validate tests, all 8 seeds and the manifest round-trip with it.
node --input-type=module -e "import { KINDS, PORT_TYPES, FLOW_KINDS } from './src/shared/graph/constants.mjs'; if (!Array.isArray(KINDS)) throw new Error('KINDS is not an array'); if (!Array.isArray(PORT_TYPES)) throw new Error('PORT_TYPES is not an array'); if (!Array.isArray(FLOW_KINDS)) throw new Error('FLOW_KINDS is not an array'); console.log('shapes OK');"
```
  `LIMITS` must expose `maxNodes: 80` and `maxWires: 200` (P1 ships `{maxNodes: 80, maxWires: 200, maxPortsPerSide: 8, minArity: 2, maxArity: 8, maxCycles: 20, maxNameLen: 80}`) — verify, do not add; any other value is a P1 regression to fix in P1, never a P2 deviation.
...
- [ ] Step 6: `npm test 2>&1 | grep -E '^ℹ (tests|pass|fail) '` — record the printed `ℹ pass` count as BASELINE; it must be green before you start (Node 25's `spec` reporter prints an 8-line summary, so `tail -5` hides the `ℹ pass` line — always use this grep).
  Expected: `ℹ fail 0` (the dry-run measured BASELINE = 3760 on dev `e6968e15`, 3810 with P1 v2 applied; write the number YOUR run prints).
...
**Interfaces produced:** `flowPorts(node)`, `portsFnFor(agentsByKey)`, `portsOf(portsFn, node) → {known, ported, inputs, outputs, meta}` (never throws), `findPort(ports, portId, dir)`, `typeCompatible(outType, inType)`, `resolveOrOutType(tpl, portsFn, orId, seen)`, `inboundWires(tpl, nodeId, portId?)`, `outboundWires(tpl, nodeId, portId?)`, `firedOutputs(ports, verdict)`. `FLOW_KINDS` is CONSUMED (not merely imported): `flowPorts` returns `undefined` for any kind outside it via `FLOW_KINDS.includes(kind)` — P1's `FLOW_KINDS`, like `KINDS` and `PORT_TYPES`, is a frozen ARRAY, so membership is `.includes`, never `.has`.
...
`Expected: ℹ pass 8` / `ℹ fail 0` (`node --test test/graph-ports.test.mjs`, and `node --test test/shared-graph-purity.test.mjs` still green)
...
```
`Expected: ℹ pass 7` / `ℹ fail 0` (`node --test test/graph-loops.test.mjs`)
...
**Consumes:** `constants.mjs` (`KINDS`, `NODE_ID_RE`, `LIMITS`), `ports.mjs`, `loops.mjs`. (`PORT_TYPES` is NOT consumed here — the validator never enumerates the type vocabulary; `agent-meta.mjs` (Task 7) owns that check.) Of `LIMITS`' seven keys this module reads FOUR: V1 enforces `maxNodes`/`maxWires` and V12 enforces `minArity`/`maxArity`; `maxPortsPerSide` belongs to `gatePorts` (P1), and `maxCycles`/`maxNameLen` are reserved for the composer (P5) and stay unread here.
...
| V2 | E | `node id <json> must match /^n_[a-z0-9]{1,32}$/` (P1's `NODE_ID_RE`; the message interpolates `${NODE_ID_RE}` so it can never drift) · `duplicate node id '<id>'` · `node '<id>' must have finite x/y coordinates` |
| V3 | E | kind ∈ `KINDS`; `key` iff agent: `node '<id>' has unknown kind <json> — expected one of agent, task, end, and, or, combine` (the list is `[...KINDS].join(', ')` — P1's declaration order, NOT alphabetical) · `agent node '<id>' must declare a key` · `node '<id>' of kind '<kind>' must not declare a key` |
...
| V9 | E | `required input '<node>.<port>' is unwired` — every required NON-LOOP input of an AGENT node; loop inputs exempt. Flow cards are OUT of scope even though P1 marks `inK`/`result` `required: true`: V12 owns an unwired `inK` and V21 owns an unwired `end.result`, each with its own sentence, so a defect is reported once |
...
test('V5 endpoints and declared ports', () => {
  ...
  gk.wires.push({ id: 'w_ghost_in', from: { node: 'n_ts', port: 'task' }, to: { node: 'n_ghost', port: 'nope' } });
...
```
`Expected: ℹ pass 23` / `ℹ fail 0` (`node --test test/graph-validate.test.mjs`)
- [ ] Step 3: Mutation audit — delete each rule's `check` body in turn (or splice the entry out of `RULES`) and confirm `node --test test/graph-validate.test.mjs` FAILS; restore. Any rule whose removal keeps the suite green needs a sharper assertion before you move on. All 21 whole-rule removals were measured caught. Then run these NINE fine-grained mutations, each of which was a SURVIVOR before this task's fixtures were sharpened — every one must now go RED:
  1. V3's `KINDS.includes(n.kind)` → the Set-style membership call ⇒ EVERY test dies with `TypeError: … is not a function` (P1's `KINDS` is a frozen ARRAY — this is the single mistake that cost the first dry-run its whole Task 3; measured `pass 1 / fail 22`).
  2. V5: drop the `metaOf(fromId) &&` guard OR the `metaOf(toId) &&` guard — EACH on its own ⇒ the `n_ghost` case reports a V5 port message (`'n_ghost.nope' is not a declared input` for the toId half) on top of V4; dropping only one half must go RED by itself (the fixture wires both out of and into the unknown-key node).
  3. V7: iterate `liveWires` instead of `wires` ⇒ the stacked `n_ghost.in` wires stop reporting.
  4. V9: drop `n.kind !== 'agent'` ⇒ the unwired `n_and.in2` reports under V9 as well as V12.
  5. V9: drop the whole `inp.loop || isLoopInput(n.id, inp.id)` clause (the two are equivalent — `loopInputs` is built from the same `inp.loop` flag — so BOTH halves must go) ⇒ the unwired required `impl.fix` reports.
  6. V11: `if (!metaOf(id)) return true` → `return false` ⇒ the `n_x` cycle reports a deadlock.
  7. V15: drop `metaOf(n.id) &&` from the entry filter ⇒ `n_x` stops warning unreachable.
  8. V18: drop the `inp.id === AWAIT_PORT_ID || inp.synthetic` skip (c) ⇒ `aw` warns; drop the `nodeById.get(w.from.node)?.kind === 'task'` skip (a) ⇒ `ts` warns; drop `inp.loop || isLoopInput(...)` (d) ⇒ `lp` warns.
  9. V19: drop the `end`/`result` exemption ⇒ `er` warns; drop the `agent`/`await` exemption ⇒ `ag` warns.
  Also confirm the V12 upper bound: change `arity > limits.maxArity` to `false` ⇒ the `arity: 99` case goes green (i.e. the assertion must go red).
...
```
`Expected: ℹ pass 10` / `ℹ fail 0` (`node --test test/graph-template.test.mjs`)
...
**Frozen constants:** `NODE_W 220, HEAD_H 34, ROW_H 24, SEP_H 9, PAD_T 8.5, PAD_B 8, BORDER 1.5, DOT 10, FOOT_H 26, EXEC_ROW_H 22, SNAP 11, PORT_HIT_R 14, WIRE_HIT_TOL 6, ZOOM_MIN 0.4, ZOOM_MAX 1.6, ZOOM_K 0.002`, `ROW0 = BORDER + HEAD_H + PAD_T + ROW_H/2 = 56`. All of them come from the reference prototype EXCEPT `WIRE_HIT_TOL 6`, which comes from spec §7.4 (the prototype hit-tests ports only, so it has no wire tolerance to borrow).
**Closed forms (the tests pin them):** zones top→bottom `inputs → outputs → await (kind 'agent' only) → caption (task/end/or)`; `nodeSize = 2·BORDER + HEAD_H + PAD_T + rows·24 + seps·9 + PAD_B + footer`, `footer = rows ? FOOT_H + (rows−1)·EXEC_ROW_H : 0`; Agent(nIn,nOut) = `95.5 + 24·(nIn+nOut)`; Task/End = `110.5`; arity-2 OR = `167.5`; arity-2 AND = `134.5`. Anchors: input i `(x, y+56+24i)`; output j `(x+220, y+65+24·nIn+24j)`, and on a 0-input card `(x+220, y+56+24j)`; await `(x, y+74+24·(nIn+nOut))`, and on a 0-input agent `(x, y+65+24·nOut)`. **TWO 0-input deviations from the spec's closed forms, both consequences of the same zone rule (a zone is emitted only when non-empty, and a 9px separator sits only BETWEEN emitted zones) — the code is right and the closed form is the approximation:** a 0-input agent is `110.5` high, not `119.5`, and its `await` sits at `y+65+24·nOut`, not `y+74+24·nOut`. Both are pinned below so P5's CDP script cannot "correct" them. The executions footer is the bottom-most box, so anchors NEVER move when it expands.
...
```
`Expected: ℹ pass 10` / `ℹ fail 0` (`node --test test/graph-geometry.test.mjs`)
...
```
`Expected: ℹ pass 7` / `ℹ fail 0` for the pair (`node --test test/graph-layout.test.mjs test/graph-thumbnail.test.mjs` — 5 layout + 2 thumbnail)
...
**One INTENTIONAL divergence from v1 leniency:** a v2 sidecar that omits `runnerType` is an ERROR here (`runnerType must be one of producer, verifier, clarifier`), where v1's `normalizeMeta` silently defaults it to `'producer'`. That is rule-for-rule with the old branch and with adj-f2 §3, and P7's port editor always writes the field, so nothing in the product can produce a v2 sidecar without it — do NOT "restore" the v1 default.
...
```
`PORT_TYPES` is a frozen ARRAY in P1 (like `KINDS` and `FLOW_KINDS`), so the spread is a defensive copy and stays as written — never call `.has` on it. `MAX_PORTS_PER_SIDE` must be 8, and `PORT_ID_RE` is imported, never redeclared.
`Expected: ℹ pass 11` / `ℹ fail 0` (`node --test test/graph-agent-meta.test.mjs`)
...
```
`Expected: FAIL — SyntaxError: The requested module '../src/core/claude-runner.mjs' does not provide an export named 'MOCK_ROLE_CLARIFY'` (Node names the FIRST unresolved binding of the import list)
...
```js
    case MOCK_ROLE_DECOMPOSER:
      ...
      break;
```
`Expected: ℹ pass 2` / `ℹ fail 0` (`node --test test/mock-writer-roles.test.mjs`). Then re-run every OTHER existing mock suite — `ls test | grep mock` prints exactly four on dev @ `e6968e15`:
```bash
node --test test/claude-runner-ask-mock.test.mjs test/skill-mock.test.mjs \
     test/subagent-mock.test.mjs test/workspace-mock.test.mjs
```
`Expected: ℹ fail 0` (21 passing across the four on dev `e6968e15`). There is no `test/mock-runner.test.mjs` and no `test/mock-graphify.test.mjs` — do not go looking for them.
...
**Three deliberate deltas vs `old:agents/*.meta.json`** (all three are spec §6, not transcription slips — do NOT "restore" the old-branch values): (1) `decomposer`'s `tasks` output gains `"artifactKind": "decomposition"`; (2) `implementer`'s inputs are REORDERED `plan, fix, task` → `fix, task, plan` and the `plan` port gains a `directive` (the single-directive rule A3 renders the first fresh directive port in DECLARED order, and v1 precedence is fix > task > implement); (3) `manualTestsChecklist.promptHints` templates the literal `` via `git diff` in your cwd `` as `{diffInstruction}`. The other **8** sidecars' port bodies are byte-identical to the old branch.

**Per-sidecar additions** (`+` = add this key; the port bodies are the §6 table, verified against dev's `phases.mjs` builders). **WHERE to put a fragment: immediately after the file's `"key": "<key>",` line** (line 2 in all 11 files on dev @ `e6968e15` — verified). Every fragment already ends with a trailing comma, so inserting it there leaves the rest of the file byte-identical; appending after the last key instead would add a comma to the previous line and break Step 3's `git diff` check:
...
**The `.md` bodies.** Seven files carry a `## Inputs (from the task prompt)` section — REPLACE that heading and its bullet list with the `## Ports` block below (everything after it — verdict/clarify JSON contracts, fan-out, workspace and graph-tooling sections, frontmatter — stays VERBATIM). Four files have no such section — INSERT the block after the intro paragraph, followed by ONE blank line before the next `## …` heading (byte-consistent with the seven replacements), before the next heading. Every block opens with the same sentence.

**Insertion anchors for the four (verified against dev @ `e6968e15` — re-check before writing, and anchor on the QUOTED heading, not the number):**

| file | insert the block … | anchor line below it |
|---|---|---|
| `agents/worca-cc-clarify.md` | after the intro paragraph (`:8`) and its blank line | `## Cardinal rule: NEVER ASSUME, NEVER PAD` (`:10`) |
| `agents/worca-cc-planner.md` | after the intro paragraph (`:8`) and its blank line | `## Fan-out (parallel sub-agents) — USE IT when enabled` (`:10`) |
| `agents/worca-cc-implementer.md` | after the intro paragraph (`:8`) and its blank line | `## Cardinal rule: FOLLOW THE PLAN` (`:10`) |
| `agents/worca-cc-decomposer.md` | after the `# Your role` paragraph (`:10-13`) and its blank line — this file opens with an H1 at `:8`, so the block goes BELOW it, never above | `## Draft vertical slices` (`:15`) |
...
for (const f of readdirSync('agents').filter((x) => x.endsWith('.meta.json'))) {
  ...
  // Dual shape: every v1 field must SURVIVE the edit. A missing one means the
  // fragment replaced keys instead of being inserted after "key".
  for (const k of ['consumes', 'produces', 'connectsTo', 'order', 'runnerType']) {
    if (!(k in raw)) { bad += 1; console.log(f, 'lost v1 field', k); }
  }
}
...
- [ ] Step 4: Commit — `worca: Node-graph v2 P2 — dual-shape meta v2 sidecars for the 11 builtins`
  The commit BODY must record the one live-behaviour change in this task, so a later bisect of a prompt regression lands here: "v1 system prompts for clarify/planner/refiner/planReviewer/decomposer/implementer/reviewer/workspaceReviewer/manualTestsChecklist/manualWebUiTesting/workspaceScanner gain/replace a `## Ports` section; task prompts unchanged." (Nothing pins those bytes — verified: no test or src file references the `## Inputs (from the task prompt)` heading — and the JSON additions are inert for v1, because the only builders that read `promptHints` are the two GENERIC ones at `phases.mjs:1202,1237`, which serve custom keys, never the 11 builtins.)

---

### Task 10: registry merge, `registryPortsFn`, the real-sidecar test helper

**Files:** modify `src/core/agent-registry.mjs` (`normalizeMeta`, `:189-248`), create `src/core/graph/registry-ports.mjs`, `test/helpers/graph-ports.mjs`; modify `test/agent-registry-schema-v2.test.mjs` (the `:90-99` block `test('the 11 shipped sidecars are unchanged by v2 (backward compatibility)')`, whose `:95` line asserts `promptHints === ''` for all 11 builtins, MUST be rewritten — SIX builtins now carry hints), create `test/graph-registry-ports.test.mjs`.

Why the merge and not "unknown keys are ignored": `normalizeMeta` returns a FIXED key set and `agent-store.updateAgent` round-trips the merged raw through it (`agent-store.mjs:97-99` — `const base = { ...existing }; if (base.descriptionDerived) base.description = ''; const raw = { ...base, ...rawMeta };`), so a user's v2 sidecar would silently LOSE its ports on the next save. `GET /api/agents` (`ui/server.mjs:3907-3913`) passes normalized metas through untouched, so the v2 fields ride to the browser with NO server edit — verify, do not change it.
...
```
(`join` is already imported at the top of the file; add the four new import lines beside the existing ones (a second `import … from 'node:fs'` next to the existing one is legal) and DELETE the old `test('the 11 shipped sidecars are unchanged by v2 (backward compatibility)')` block at `:90-99` — the test above replaces it.)
...
```
`Expected: ℹ fail 0` on `node --test test/agent-registry-schema-v2.test.mjs test/graph-registry-ports.test.mjs`. Six test blocks are newly WRITTEN here (4 + 2) but the suite's NET delta is **+5**: `test/agent-registry-schema-v2.test.mjs` gains 4 and loses 1 (the `:90-99` backward-compat test is rewritten, not added beside), `test/graph-registry-ports.test.mjs` adds 2. Also `node --test test/agent-registry.test.mjs test/agent-registry-decomposer.test.mjs test/agent-registry-workspace.test.mjs test/agent-registry-layered.test.mjs test/agent-registry-desc-fallback.test.mjs test/agents-meta.test.mjs test/agents-api.test.mjs test/api-agents-domain.test.mjs` stays green (those suites deepEqual only v1 fields).
- [ ] Step 5: Verify `GET /api/agents` needs NO edit — the v2 fields must ride through untouched:
```bash
node --test test/agents-api.test.mjs test/api-agents-domain.test.mjs
grep -n "res.json({ agents" ui/server.mjs        # :3928 after P1's 15-line /src/shared mount insert (:3913 on bare dev) — objects pass through; no field list to update
```
...
export function buildGraphManifest(tpl, agentsByKey, opts = {}) {
  ...
  const manifestNodes = nodes.map((node) => {
    ...
    const cell = {
      ...
      // field (`workflows.mjs:412`), so dropping it would strand every custom
...
```
`Expected: ℹ pass 9` / `ℹ fail 0` (`node --test test/graph-manifest.test.mjs`)
...
**Files:** create `ui/public/graph/model.mjs`, `test/shared-graph-single-source.test.mjs`. P1's purity guard (`test/shared-graph-purity.test.mjs`) has a **third** test — `ui/public leaves the static root only into src/shared, at the URL the mount serves` (`:46`; the first two are the never-vacuous check `:21` and the purity sweep `:28`) — that walks every `ui/public` specifier leaving the static root; until now no such specifier existed, so this task gives it a REAL one — after this task, breaking the import convention fails that guard.
...
`Expected: ℹ pass 2` / `ℹ fail 0` (`node --test test/shared-graph-single-source.test.mjs`); `node --test test/shared-graph-purity.test.mjs test/api-shared-static.test.mjs` stays green (the purity guard now has a real specifier to check, and the static mount serves every new shared file).
...
```
  (`SEED_TEMPLATES`, `FB_WIRE_MAP` and `GRAPH_DEFAULT_WORKFLOW` are already imported by P1's version of this file; add only the three new imports, BESIDE P1's imports at the TOP of the file — not appended at the bottom. If P1 imported them under different local names, use those.)
...
`Expected: ℹ fail 0` with **3 more** `ℹ pass` than P1's version of this file printed (P1's structural pins stay). If a seed reports an error, the FIRST suspects are: a sidecar port id that does not match the seed's wire (`V5`), a `maxCycles` on a non-loop wire (`V13`), or a missing `metaVersion: 2` (`V4`).
...
- [ ] Step 1: `npm test 2>&1 | grep -E '^ℹ (tests|pass|fail) '`  (Node 25's `spec` reporter prints an 8-line summary — `tail -5` hides the `ℹ pass` line.)
`Expected: ℹ pass BASELINE + 97` / `ℹ fail 0` — **re-measured in the wave-2 execution; write the printed number.** With Task 0's BASELINE of 3810 (dev `e6968e15` + P1 v2) that projects to **3907**. The 97 net-new P2a tests, per file:
...
| `test/agent-registry-schema-v2.test.mjs` | **+3** (4 added − 1 rewritten away) |
...
Every sharpened fixture this plan adds (V5/V7/V9/V11/V15/V18/V19 exemption cases, the manifest `config` pins, the geometry await anchor) rides INSIDE an existing `test(` block, so the count is unchanged from the v1 dry-run. If yours differs, reconcile BEFORE committing — a missing test is a missing rule.
...
`Expected: ℹ fail 0` (every new `src/shared/graph/*.mjs` is pure and GETs 200 `application/javascript` with nosniff).
- [ ] Step 3: Commit the half marker — `git commit --allow-empty -m "worca: Node-graph v2 P2 — P2a green (shared core + sidecars)"` (every task above already committed its own files, so this is a deliberate EMPTY marker commit that a later `git log` bisects the half boundary on; drop it if your workflow dislikes empty commits).

### — split point: P2b starts here —


> **P2b entry note.** P2b depends on ALL of P2a (the shared modules, `registry-ports.mjs`, the merged registry with `inputs/outputs` on the 11 builtins, and the ported sidecars — every `resolveGraph` would otherwise throw `agent "planner" has no v2 ports`); it does NOT depend on P1's harness split. It does NOT depend on P1's harness split. If you are executing P2b as its own pipeline run, start at Task B0.

### Task B0: P2b entry check (branch, deps, P2a sentinel, baseline)

> Anchor note: every `ui/server.mjs` line number quoted in P2b is the dev @ `e6968e15` number; with P1 applied each is exactly **+15** (P1 Task 8 inserts the 15-line `/src/shared` mount at `:766`) — e.g. `POST /api/run` 1062 → 1077, `GET /api/workflows` 3116 → 3131, `PATCH /api/config` 2751 → 2766. Locate by the quoted text, not the number.
...
```
- [ ] Step 4: `npm test 2>&1 | grep -E '^ℹ (tests|pass|fail) '` — record the printed `ℹ pass` count as BASELINE-B (must be green; `tail -5` hides that line under Node 25's 8-line summary). It is Task 14's number — the v1 dry-run measured 3902 with P1 v1 applied, which projects to **3907** with P1 v2; write what YOUR run prints.

---

### Task B1: DB **V23** — additive schema, `SCHEMA_VERSION` 22 → 23

**Files:** modify `src/core/db.mjs` (`SCHEMA_VERSION` `:56`, `INCREMENTAL_COLUMNS` `:732-742`, `INCREMENTAL_TABLES` `:752-766`, the ladder `:1049-1073` — the load-bearing tail is `:1072 if (current < 22) applySchemaV22(db);` followed by the `:1073` stamp, so the new `if (current < 23) applySchemaV23(db);` goes between them), `test/db.test.mjs` (`EXPECTED_TABLES` `:74-93`, 18 entries; the title `:120` and the length assertion `:126`), create `test/db-migrate-v23.test.mjs`.

Every DDL is additive and reconcile-safe: columns go through `INCREMENTAL_COLUMNS` and the table through `INCREMENTAL_TABLES`, so `schemaGaps`/`repairSchemaGaps` heal a DB that a divergent ladder already stamped (`db.mjs:732-766` are those two maps; `schemaGaps :814`, `repairSchemaGaps :834`, `reconcileSchema :855` — the fast-path heal `applySchemaV23` reuses). No data rewrite, no backup — that is V24's job in P8.
```sql
...
- [ ] Step 4: Version-pin sweep — no test may pin the OLD number. Run BEFORE and AFTER the bump:
```bash
grep -rn -A1 "user_version" test/*.mjs | grep -v db-migrate-v23 \
  | sed -E 's/^[^:]+[:-][0-9]+[:-]//' | grep -w 22
```
  The `sed` strips grep's own `file:lineno:` (and `file-lineno-` context) prefix, WITHOUT it a line NUMBER 22 matches `grep -w 22` and the sweep can never be empty — four permanent false positives on dev: `db-pause-schema.test.mjs:22`, `subagent-migration-v6/v7/v8.test.mjs:22`. The `grep -v db-migrate-v23` must run BEFORE the `sed` (once the prefix is gone there is no filename left to filter on): the new v23 suite deliberately stamps `PRAGMA user_version = 22` to simulate an older DB.
  `Expected BEFORE the bump:` exactly these two lines, both prose comments, no assertion among them:
```
...
```
  (`test/ask-db-schema.test.mjs:80` and `test/diff-comments-schema.test.mjs:72`.) Update BOTH comments to `17 -> 23` in this task — they are prose, not assertions.
  `Expected AFTER:` **empty output.**
...
`Expected: ℹ pass 4` / `ℹ fail 0` on `test/db-migrate-v23.test.mjs`, and `ℹ fail 0` on every other suite in the list — their counts are unchanged (tests that stamp 15–21 on a minimal seed now run the v23 step too: it is a column-guarded no-op).
...
- **One text for an unknown id, everywhere.** `assertRunnableWorkflow` and `resolveGraph` (Task B4) BOTH throw `unknown workflowId "<id>"`. `resolveGraph` reaches that line only when it is called directly (every run path goes through the gate first), so the two can never race — but they must not disagree either, because P4's `_resolveTopology` maps the text. Do not reintroduce `workflow not found: <id>`.
...
```
`Expected: FAIL — SyntaxError: The requested module '../src/core/workflows.mjs' does not provide an export named 'assertRunnableWorkflow'` (Node names the FIRST unresolved binding of the import list)
...
```
  New `writeGraphWorkflow` — insert it between the end of `writeWorkflow` (`return stored;\n}`) and the JSDoc that begins `/**\n * Read a template by id. Returns the built-in DEFAULT_WORKFLOW for "wf_default";` (`workflows.mjs:284` on dev; NOT "Read one template by id.", which is `readRaw`'s lead-in). `writeWorkflow` stays v1 and untouched:
```js
/**
...
```
`Expected: ℹ pass 7` / `ℹ fail 0` (`node --test test/workflows-graph-rows.test.mjs`); `node --test test/workflows.test.mjs test/workflows-db.test.mjs test/workflows-questions.test.mjs test/api-workflows.test.mjs test/migrate-fs-to-db.test.mjs` stays green (`writeWorkflow` and the v1 read path are untouched).
...
**Files:** modify `src/core/config.mjs` (`readWorkflowsMap :550`, `setFeedbackCycles :674` — add its wire twin beside it, `resetWorkflowConfig :701`, `resolveRunConfig :747`), modify `test/run-config.test.mjs` (`:96` deep-equals the WHOLE `resolveRunConfig` return — Step 2 edit 5), create `test/config-wire-cycles.test.mjs`.
...
test('GET /api/config shape: nodes + feedbacks (v1) + wires (v2) coexist', async () => {
  // `setNodeModel` VALIDATES against listModels() (`config.mjs:400-410`) and
...
```
  5. `test/run-config.test.mjs:96` deep-equals the WHOLE `resolveRunConfig` return, so the new key must be added THERE too — replace
```js
...
```
  This file is NOT in this task's Files list by accident: without edit 5 a per-file executor passes B3 and the break only surfaces in Task B10's full suite (measured `pass 3938 / fail 1`), eight tasks later. Add `test/run-config.test.mjs` to this task's Files line and to the verification below.
`Expected: ℹ pass 4` / `ℹ fail 0` (`node --test test/config-wire-cycles.test.mjs`); then
```bash
node --test test/config.test.mjs test/config-db.test.mjs test/config-api.test.mjs \
     test/config-questions.test.mjs test/run-config.test.mjs
```
`Expected: ℹ fail 0`.
...
test('overlay precedence: run-config wins, and effort never inherits across a model change', async () => {
  ...
  // (`config.mjs:400-410, :617-627`; EFFORTS = medium/high/xhigh/max — there is
...
```
  and `setWorkflowNodeDefaults` (`:311`), immediately AFTER the line `const patch = map && typeof map === 'object' ? map : {};` (`:319`) and BEFORE `const steps = tpl.steps.map(…)` (the v2 arm reads `patch`; placing it before that line throws `ReferenceError: Cannot access 'patch' before initialization`):
```js
  if (tpl.version === 2) {
...
```
`Expected: ℹ pass 7` / `ℹ fail 0` (`node --test test/workflows-resolve-graph.test.mjs`)
...
// NOTE: dev's handler is `app.get('/api/workflows', async (_req, res) => {` —
// the replacement MUST rename `_req` to `req`, because it now reads
// `req.query.archived`. Leaving the underscore is a `ReferenceError` on the
// first request.
...
```
  with the imports extended: `assertRunnableWorkflow, writeGraphWorkflow` from `../src/core/workflows.mjs`, `registryPortsFn` from `../src/core/graph/registry-ports.mjs`, `validateGraph` from `../src/shared/graph/validate.mjs`. (`listModels`, `nodeDefaultsError`, `badRequest` and `AGENTS_DIR` are already in the module.)
`Expected: ℹ pass 6` / `ℹ fail 0` (`node --test test/api-workflows-graph.test.mjs`); `node --test test/api-workflows.test.mjs test/api-workflows-warnings.test.mjs` stays green — re-measured in the wave-2 execution; write the printed number.
...
```
`Expected: FAIL — TypeError: Cannot read properties of undefined (reading 'wires')` (the `wires` key is ignored, so `config.workflows.wf_g` is undefined), plus `actual: 200 / expected: 400` on the missing-workflowId case — `ℹ pass 7 / fail 2` for the file (B5's six still pass).
...
`Expected: ℹ pass 9` / `ℹ fail 0` for the whole file (`node --test test/api-workflows-graph.test.mjs` — B5's 6 + these 3); `node --test test/config-api.test.mjs` stays green — re-measured in the wave-2 execution; write the printed number.
...
```
  `src/core/ask/proposal.mjs` — the gate must go through the **injection seam**, not around it. `createProposalValidator({ readWorkflow })` (`:54-61`) exists so the MCP child can be driven with fake readers, and `test/ask-proposal.test.mjs:27` injects a fake `readWorkflow` that knows `wf_default`/`wf_review`; calling the module-level `assertRunnableWorkflow` directly would bypass it, hit the real (empty) DB and turn the existing suite red. So `assertRunnableWorkflow` becomes a DEP with the real import as its default. Add it to the destructured deps (`:54-61`):
```js
...
```
  and replace the workflow block (`:104-106`) with, keeping the existing error-collection shape:
```js
...
```
  (import it as `import { assertRunnableWorkflow as realAssertRunnableWorkflow } from '../workflows.mjs';` beside the existing `readWorkflow` import; also extend the `@param` typedef at `:52`. Leave `PROPOSAL_ERRORS.unknownWorkflow` in place — `assertRunnableWorkflow`'s NOT_FOUND text is the same sentence.)
  Then `test/ask-proposal.test.mjs` must inject the twin, or its `wf_review` case starts failing with `unknown workflowId "wf_review"`. Add to `deps` (`:15-31`), right after the existing `readWorkflow` line:
```js
...
```
  and add ONE new case pinning the interim refusal (survivor S6 — nothing else covers Ask's arm):
```js
...
`Expected: ℹ pass 4` / `ℹ fail 0` (`node --test test/run-workflow-gate.test.mjs`) and `ℹ fail 0` on `node --test test/ask-proposal.test.mjs` (one test MORE than before this task) — re-measured in the wave-2 execution; write the printed numbers. Every `test/api-*.test.mjs` that posts a run stays green.
...
The LLM-facing catalog shape must not change: Ask's prompt and its `list_workflows` tool render `{id, name, domain, origin, steps: [[{nodeId,key,displayName,description}]], feedbacks: [{id,from,to}]}`. For a v2 row those are DERIVED — one group per `rankNodes` rank (longest path with loop wires excluded, nodes in TEMPLATE order inside a group (the manifest shim additionally sorts a cell by launch index; the RANK definition — `rankNodes`, loop wires excluded — is the same, which is what Ask's LLM-facing shape needs); agent nodes only, flow cards are engine plumbing the assistant must not reason about), and one feedback per loop wire.
...
```
`Expected: ℹ pass 3` / `ℹ fail 0` (`node --test test/ask-catalog-graph.test.mjs`); `node --test test/ask-catalog.test.mjs test/ask-tools.test.mjs test/ask-prompt.test.mjs` stays green (run whatever `ls test | grep ^ask-` reports).
...
**Files:** modify `src/core/agent-store.mjs` (`:133`), `src/core/plugin-workflows.mjs` (`referencedPluginAgents :150-179`; the SQL to widen is `:166` — `'SELECT id, name, steps FROM workflows WHERE origin IS NULL OR origin != ?'`), `ui/public/app.js` (`buildNodeConfigRows :2669`, `buildFeedbackRows :2813`, `composerRenderList :2561`, and the `window.__composer` test hook at `:1973-1976`), modify `test/ui-composer.test.mjs` (it already boots jsdom — the behavioural guard test goes there), create `test/graph-row-consumers.test.mjs`.
...
```
  with the extra imports this file needs: Place them at the TOP of the file (ESM hoists imports, but the assembled file should read top-down).
```js
...
```
  (`pluginCurrentDir` lives in `src/core/plugins-lock.mjs:26` — `export function pluginCurrentDir(name) { return join(pluginDir(name), 'current'); }` — and is what `referencedPluginAgents` itself calls, so the fake sidecar lands exactly where the scan looks; `useTempHome` keeps it inside the sandbox. Also: this second test must run AFTER the first, which is the file's declaration order — node:test is sequential within a file.)

  The three `ui/public/app.js` guards get a BEHAVIOURAL test, not a source regex: `if (false && workflow.version === 2) return [];` and `filter((w) => w || w.version !== 2)` both keep a regex pin green (measured). `buildNodeConfigRows`/`buildFeedbackRows` are already on `window.__np` (`app.js:2879-2882`) and `composer` on `window.__composer` (`:1973`), so `test/ui-composer.test.mjs`'s existing `boot()` reaches all three. APPEND to that file:
```js
test('v2 graph rows are inert in the v1 run-setup builders and the v1 composer list', async () => {
  ...
  // `composerRenderList` reads `item.steps.length` (app.js:2533).
...
```
  and extend the REFERENCED sentence (`:137`) so the scan's new reach is legible — an agent held only by an archived template cannot be deleted until that row is deleted via `DELETE /api/workflows/:id`, and without this the message names a workflow the user cannot even see in the picker (checked: no test pins this string, `grep -rn "delete or edit those first" test/` is empty):
```js
...
```
  and, so the guard can be tested BEHAVIOURALLY rather than by a source regex, add one line to the existing `window.__composer` hook (`:1973-1976`, same convention as its neighbours):
```js
  window.__composerRenderList = composerRenderList;
```
`Expected: ℹ pass 2` / `ℹ fail 0` (`node --test test/graph-row-consumers.test.mjs`) plus ONE more test in `test/ui-composer.test.mjs` than before; `node --test test/agent-store.test.mjs test/plugin-workflows.test.mjs test/ui-composer.test.mjs test/composer-ui.test.mjs` stays green — re-measured in the wave-2 execution; write the printed numbers.
...
- [ ] Step 1: `npm test 2>&1 | grep -E '^ℹ (tests|pass|fail) '`  (Node 25's `spec` reporter prints an 8-line summary — `tail -5` hides the `ℹ pass` line.)
`Expected: ℹ pass BASELINE-B + 42` / `ℹ fail 0`, where BASELINE-B is the number Task 14 recorded — **re-measured in the wave-2 execution; write the printed number.** With BASELINE-B = 3907 that projects to **3949**. The 42 new P2b tests, per file:

| file | new tests |
|---|---|
| `test/db-migrate-v23.test.mjs` | 4 |
| `test/workflows-graph-rows.test.mjs` | 7 |
| `test/config-wire-cycles.test.mjs` | 4 |
| `test/workflows-resolve-graph.test.mjs` | 7 |
| `test/api-workflows-graph.test.mjs` | 9 (B5's 6 + B6's 3) |
| `test/run-workflow-gate.test.mjs` | 4 |
| `test/ask-proposal.test.mjs` | +1 (B7's graph-refusal case) |
| `test/ask-catalog-graph.test.mjs` | 3 |
| `test/graph-row-consumers.test.mjs` | 2 |
| `test/ui-composer.test.mjs` | +1 (B9's behavioural guard) |
| **total** | **42** |

(If you executed both halves in one run the total is **BASELINE + 139**.) `test/run-config.test.mjs` changes in B3 but gains no test.
- [ ] Step 2: Version-pin sweep, final (same command as Task B1 Step 4):
```bash
grep -rn -A1 "user_version" test/*.mjs | grep -v db-migrate-v23 \
  | sed -E 's/^[^:]+[:-][0-9]+[:-]//' | grep -w 22
```
`Expected: empty output` — B1 already rewrote the only two prose hits (`ask-db-schema.test.mjs:80`, `diff-comments-schema.test.mjs:72`) to `17 -> 23`. If anything else prints, a suite is still pinning 22.
...
- **`PORT_ID_RE`** — the old branch's `/^[a-z][A-Za-z0-9_-]{0,31}$/` or P1's strict lowerCamel? → **P1's STRICT `/^[a-z][A-Za-z0-9]{0,31}$/`, imported from `constants.mjs` by `agent-meta.mjs` — never redeclared. Spec §3's module table puts the regex in `constants.mjs` precisely so there is ONE source; two copies meant a port id like `fix_review` passed the store's 400 and failed P1's seed guard. The strict form matches the product text "Port ids are lowerCamel, ≤32 chars" (adj-f2 §2) and every builtin and flow port id; `in_1` and `in-1` are INVALID, as P1's own test already pins. **Deviation recorded:** adj-f2 §3 / base spec §5's `_-` wording is superseded (spec §15's deviation index is untouched — the spec is untracked) (Fable critique F7, cross-plan pass 2026-08-27).**
- **`NODE_ID_RE`** — base spec V2's `/^[A-Za-z0-9_-]{1,64}$/` or P1's `n_`-prefixed space? → **P1's `/^n_[a-z0-9]{1,32}$/`: the composer mints `n_` + 8 base36 and the seeds use `n_<word>`. V2's message interpolates `${NODE_ID_RE}` so it can never claim a rule the code does not enforce. **Deviation recorded:** the tighter id space is a deliberate P1 decision against base spec V2 (Fable critique F8).**
- **`promptHints` on the builtins** — five or six? → **SIX: `implementer, manualTestsChecklist, manualWebUiTesting, planReviewer, refiner, workspaceReviewer`. The implementer's hint ("Work inside the project directory (your cwd). Commit nothing; just edit files and tests.") is spec §6's row (`phases.mjs:836`) and P3's parity pin (adj-f1 §5), so it MUST be in the sidecar; the five-item assertion in the v1 draft would have failed on a correct implementation. This changes NO v1 prompt byte: only the two GENERIC builders (`phases.mjs:1202,1237`) read `promptHints`, and they serve custom keys, never the 11 builtins (Fable critique F3, dry-run D2).**
- **Manifest `node.config`** — rebuild it key by key, or carry it? → **Carry it VERBATIM: every manifest cell gets `config: { ...node.config }` and `manifestTemplate` restores it as-is. The manifest is the ONLY persisted copy of the topology (spec §15: a v2 resume never re-reads the workflow row), so the v1 draft's `{arity, awaitAll}` rebuild silently dropped `wf_provided-plan`'s `planStoreSeed` and every future tunable. **Spec §5.8's node-cell shape gains `config` (additive).** P4's `resolvedFromManifest` then reads `mn.config` instead of `{ arity: mn.arity }` (Fable critique F9).**
- **V9's scope** — every required input, or agent inputs only? → **AGENT inputs only. P1 marks gate `inK` ports and `end.result` `required: true`, and V12/V21 already own them with card-specific sentences, so without the guard one unwired `n_and.in2` reports twice in the 422 body (Fable critique F13).**
- **V12's arity bound** — `>= 2` only, or both ends? → **BOTH: `LIMITS.minArity`..`LIMITS.maxArity` (2..8). `gatePorts` clamps to `MAX_PORTS_PER_SIDE`, so an unbounded rule stored `arity: 99`, shipped it in the manifest and rendered only `in1..in8` — an unwirable card with no error (Fable critique F14).**
- **Ask's runnable gate** — call `assertRunnableWorkflow` directly, or inject it? → **INJECT it: `createProposalValidator` gains `assertRunnableWorkflow = realAssertRunnableWorkflow` beside its other reader deps. `src/core/ask/proposal.mjs:54-61` is an injection seam the MCP child and `test/ask-proposal.test.mjs:27` both drive with fakes; a direct module-level call bypasses the fake reader and turns the existing suite red (Fable critique F6, dry-run D10).**
- **`writeGraphWorkflow`'s `origin` on conflict** — overwrite or keep? → **`origin = COALESCE(excluded.origin, workflows.origin)`. The composer's Save on a loaded row sends `{id, name, nodes, wires}` and no `origin`, and v1's `writeWorkflow` never touches the column on conflict, so a plain overwrite silently detached plugin-owned `wfp_*` rows from `removePluginWorkflows`' guard (Fable critique F11).**
- **`POST /api/workflows` v2 branch** — validate topology only, or the catalog too? → **The catalog too, exactly like the v1 branch (`ui/server.mjs:3150-3156`): a v2 node's `config` IS its defaults block, so `nodeDefaultsError` runs over the 4 tunables before `validateGraph` and answers 400 with the same texts as `PATCH …/defaults` (Fable critique F12).**
- **`runnerType` on a v2 sidecar** — default to `producer` like v1, or require it? → **REQUIRE it. `agent-meta.mjs` is stricter than v1's `normalizeMeta` here, rule-for-rule with the old branch and adj-f2 §3; P7's port editor always writes the field, so nothing in the product can emit a v2 sidecar without it (Fable critique F24).**
- **P2-aa** — Manifest OR/AND cells: what type do the `inK` inputs carry? → **`inK` inputs are emitted as `type:'any'` with `required:true, loop:false, expands:false`; only `out` carries the RESOLVED type (Amendment f §6 — "OR's out row captions the resolved type; inK rows stay any"), so `manifestPortsFn` round-trips. P6's renderer follows THIS cell shape, not the spec §5.8 sample (agent adjudication, fresh-eyes G8; recorded as a §5.8 deviation).**
