# Ask Worca — P1 Core & Runner Implementation Plan (v3)
...
**Revision:** v3 (2026-08-22) — v1 reviewed by four agents (anchor check, two executed TDD dry-runs incl. the real fixture capture, Fable adjudication) → v2; v2 then re-executed end to end (all 18 tasks, zero code fixes, suite 3127/3127, fresh capture $0.078) and cold-reviewed by a second Fable pass; everything folded in here; see "Self-review" items 5–6.
...
- **Anchors:** every `file:line` in this plan is against `dev` @ `79dc9256` BEFORE any task has been applied (T1 alone adds ≈ 35 lines to `claude-runner.mjs`, so T16's numbers shift); the anchor is the quoted text, the number is a hint.
...
| `ui/server.mjs` | modify (3 hunks: `:30-36` import, `:2159-2165`, `:2175-2210`) | settings import, `settingsState()` keys, POST normalisation/validation/setters, legacy root guard (T5) |
...
T1 runner options ──┬─ T2 title pass-through
                    ├─ T14 spawn.mjs ──────────────────────────┐
                    └─────────────────────┐                    │
T3 listProjects key ─┬─ T9 catalog ─┬─ T10 prompt (+T5)        │
                     └─ T11 proposal (+T5) ─┐                  │
T4 db schema ── T8 store ───────────────────┼─ T12 tools ── T13 mcp-stdio ──┐
T5 settings + limits ───────────────────────┤                               ├─ T17 fixtures ── T18 docs
T6 redact ──────────────────────────────────┼─ T15 events ─┬─ T16 mock ask role (needs T1 + T15)
T7 models (independent)                     ┘              └────────────────┘
```

Waves for parallel execution: **W1** {T1, T3, T4, T5, T6, T7} → **W2** {T2, T8, T9, T11, T14, T15} → **W3** {T10, T12, T16} → **W4** T13 → **W5** T17 → **W6** T18. T16's test imports the reducer (T15) — it must not start before T15 is green. Each task ends with its own test file green AND `npm test` green.
...
```bash
git checkout dev && git pull --ff-only   # skip the pull when offline or without an upstream
git checkout -b worca/ask-worca
...
**Files:**
- Modify: `src/core/claude-runner.mjs:232-251` (runClaude destructure), `:267-285` (runReal call), `:338` (runReal params), `:340-343` (inner buildClaudeArgs call), `:308-336` (buildClaudeArgs), `:194-224` (JSDoc — the new `@param`s go before `@returns` at `:224`)
...
Add the eight names after `resumeSessionId,` and before `bin = DEFAULT_BIN,` (the unique anchor is that PAIR of lines — `workspaceWriteTargets,` sits between `modelEnv,` and `resumeSessionId,`; do not anchor on `modelEnv,`):
...
- [ ] **Step 7: Document the options in the `runClaude` JSDoc (`:194-224`, before the `@returns` line)**

Insert after the line `` *   Never reaches argv: `runReal` ignores it by construction.`` (the end of the `workspaceWriteTargets` @param) and before `` * @returns {Promise<{text:string, exitCode:number}>}``:
...
Update the `removeProject` JSDoc to the same shape: replace the two-line pair `` * @param {string} name`` + `` * @returns {Promise<Array<{name:string, path:string, exists:boolean}>>}`` (`:146-147`) — the `@returns` line alone is NOT unique (`addProject` carries the same one).
...
In `src/core/db.mjs`, immediately after the `` `; `` that closes `MODEL_COST_FLAGS_DDL` (`db.mjs:551`), one blank line, before `const SCHEMA_V11 = ```, add:
...
**1. Version** — `db.mjs:54`: `const SCHEMA_VERSION = 18;`
...
# test/migrate-v15.test.mjs:58 is a test TITLE ('… version stays 17') the guarded perl above skips:
perl -pi -e 's/version stays 17/version stays 18/' test/migrate-v15.test.mjs
# verify: no stamp assertion still says 17 (the new ask-db-schema test legitimately stamps 17 in its ladder case)
grep -n "user_version" test/*.test.mjs | grep -v ask-db-schema | grep -E "\b17\b" ; echo "exit=$? (1 = clean)"
...
- Modify: `ui/server.mjs:30-36` (import), `:2159-2165` (`settingsState`), `:2175-2210` (`POST /api/settings`)
...
Append directly after the closing `}` of `assertCostLimitInputs` (`settings.mjs:385`), one blank line, before the `/** Write (or clear) a USD cap key. …` comment:
...
In `test/settings-projects-root.test.mjs`, replace the two key-list lines at `:312-313` — today:
...
and add, after the `});` (`:322`) that closes the test ending `assert.equal(j.costLimitResetPeriod, 'monthly', 'the default window');`, the regression the spec requires (pattern: `test/budget-api.test.mjs:90-99`) — the multi-key 400 proves the SET validation runs before any write:
...
- [ ] **Step 7: Implement the three `ui/server.mjs` hunks** (each is a whole-region replacement: the entire settings import block; the entire `const settingsState = () => ({…});`; the entire `app.post('/api/settings', …)` handler)
...
At `src/core/artifacts.mjs:1352` change `function totalsFor(row) {` to `export function totalsFor(row) {` and replace its JSDoc line `` * The pipeline's {cost, active} totals for the history list, read from the DB row.`` with `` * The pipeline's {cost, active} totals for the history list (and the Ask Worca`` / `` * `get_run` tool, ask-worca-design.md §6.4), read from the DB row.`` After `lookupPipelineRow` (`:1753`) add:
...
Run: `WORCA_HOME=/tmp/worca-p1-home node --disable-warning=ExperimentalWarning --test test/ask-mcp-stdio.test.mjs`
Expected: PASS (the two child tests take ≈ 100 ms each — measured 90 / 61 ms in the dry run).
...
ASK_MCP_SERVER_PATH   // absolute path of src/core/ask/mcp-stdio.mjs — what P2 passes as `serverPath`
...
// settle() → Promise<void>: awaits every value returned by onProposal (P2's async validation) — call it BEFORE finish(); addBlock/updateBlock after finish() return null and count in reducerErrors.
...
**Files:**
- Modify: `src/core/claude-runner.mjs` — the top of `runMock` (`:689-700` before T1; anchor on the text), the `runMock({...})` call in `runClaude` (`:264` before T1) and the `if (m.MOCK_ASK) {` arm (`:707` before T1), plus a new `mockAsk` helper inserted between `emitMockSubAgents` and `function abortIfNeeded(signal) {`
...
- [ ] **Step 3: Implement — the dispatch at the top of `runMock` (the `async function runMock({…})` line, `claude-runner.mjs:689` before T1)**
...
- [ ] **Step 3b: Export `mockEnabled`** — change `function mockEnabled(opts) {` (`claude-runner.mjs:188` before T1, ≈ `:196` after) to `export function mockEnabled(opts) {`. P2 uses it to decide whether a turn runs in mock mode (`turn.mock = {card}`); nothing else changes.

- [ ] **Step 3c: Harden the legacy `MOCK_ASK` arm under `dontAsk`** (rule R-F: even if a server spawn forgets the ask markers, a `dontAsk` spawn — the Ask Worca recipe — must never write a questions file). Two one-line edits: in `runClaude` change `    return runMock({ cwd, systemPrompt, prompt, onEvent, signal, resumeSessionId, workspaceWriteTargets });` to `    return runMock({ cwd, systemPrompt, prompt, onEvent, signal, resumeSessionId, workspaceWriteTargets, permissionMode });` (the mock call is a GATE like `runReal`'s — the field must be named here), and change the arm `  if (m.MOCK_ASK) {` to `  if (m.MOCK_ASK && permissionMode !== 'dontAsk') {   // a dontAsk spawn (the Ask Worca recipe) never writes a questions file, markers or not`. Pipeline roles keep `acceptEdits` (the default), so their behaviour is unchanged.
...
**Interfaces:**
- Consumes: `runClaude` + options (T1), `buildAskSpawnOptions` / `buildMcpConfig` / `ASK_MCP_SERVER_PATH` (T14), `buildSystemPrompt` + `buildCatalog` (T9/T10), `createThread` (T8), the seed helpers, the MCP child (T13), `redactAskText` (T6); the replay test consumes `createTurnReducer` (T15).
...
`package.json` → add `"ask:fixtures": "node --disable-warning=ExperimentalWarning scripts/ask-capture-fixtures.mjs",` to `scripts`, as the line after `"smoke:plugin"` and before `"test"` (`git diff package.json` shows exactly one added line).
...
Expected: seven `captured <name>: …` lines, the table, `total cost ≈ $0.08` (measured 2026-08-22: $0.0655 and $0.0779 on two runs — the header's ≈ $0.15 is an upper bound), and `test/fixtures/ask/` containing 14 files. **The model is not deterministic:** `task-subagent` asks haiku for exactly one `Task` call and haiku sometimes issues two; the script then prints `[ask-capture] task-subagent: expected one Task/Agent spawn, got 2 — retrying (1/3)` and captures again (each attempt is paid for). If all three attempts fail, re-run just that scenario: `node --disable-warning=ExperimentalWarning scripts/ask-capture-fixtures.mjs --only task-subagent` (per-scenario captures compose: the out dir is never wiped and id numbering restarts per file). A failure naming a **recipe** problem never retries: "did not finish in the FOREGROUND shape" means `CLAUDE_CODE_DISABLE_BACKGROUND_TASKS` did not reach `claude` — check `buildAskSpawnOptions().modelEnv` and `prepareModelEnv` (`model-env.mjs`); "recipe violation" / "worca MCP not connected" / "unexpected tools" point at the spawn options. Never hand-edit a fixture — fix the recipe and re-run.
...
**Files:**
- Modify: `docs/storage.md:11-21` (the fenced layout block)
...
Replace the fenced layout block (`:11-21`, fence to fence) with:
...
- [ ] **Step 2: `docs/guardrails.md` — the Ask Worca sandbox paragraph (append as the last bullet of "Honest limitations", i.e. after the bullet ending `capability probe. In-run title generation IS scrubbed.`)**
...
Run: `npm test 2>&1 | tail -5`
Expected: `# fail 0`; the count grows by the 19 new files (+122 tests measured on the v2 re-execution: 3005 → 3127). `git status` shows nothing untracked except `docs/superpowers/` (never committed) and the usual `marketing/`.

---

## Verification ledger (P1)

Planning-time probes on `claude` 2.1.239 (2026-08-22: A2's 14 runs ≈ $0.20 + the `R6-title` run $0.0147) closed the following spec §14 rows; the plan's own dry run then captured the 7 fixtures for $0.0655 with every capture-time assertion green. The remaining manual-gate work for P4 is listed underneath.
...
| Task `tool_use_result` object shape | VERIFIED **only with `CLAUDE_CODE_DISABLE_BACKGROUND_TASKS=1`** | T14 sets it AND proves it on the spawned env (fake-bin env dump, offline); the live foreground shape is asserted only while `npm run ask:fixtures` runs (T17) — the replay test asserts the captured shape |
| `mcpServers.<name>.env` merged over the scrubbed env | VERIFIED — gate (2) closed | T13/T14 also pass `--home/--thread` argv twins |
| sub-agents inherit the tool pool | VERIFIED (Explore child has no Read; deny-RULE inheritance by a child not separately exercised) — gates (3)(4) closed | T14 `--tools Task` + SANDBOX_NOTE; T17 capture asserts the foreground child pool |
| `~/.claude/CLAUDE.md` loaded under `--setting-sources project` | not loaded per the model's self-report + zero hint hits (not a hard proof) | spec §6.3 "what still enters" list amended to "not loaded, not guaranteed" |
...
| `--tools ""` semantics; `--strict-mcp-config` without `--mcp-config` (title call) | VERIFIED (probes P9 and `R6-title`) | T1/T2 |
...
3. **Type consistency (re-verified by the v1 review wave: every import resolves, every anchor OK or fixed here):** names used across tasks were cross-checked — `createTurnReducer` (T15) is what T16/T17 import; `buildAskSpawnOptions`/`buildMcpConfig` (T14) are what T17 calls; `defaultToolDeps` (T12) is what T13 uses; `readAttachmentText(threadId, id)` (T8) is what `tool-deps.mjs` binds; `askMaxTurns`/`askMaxBudgetUsd` (T5) are what `limits.mjs` reads and `ui/server.mjs` imports; `findPipelineRowById` (T12) — singular — replaces B1's draft name `findPipelineRowsById` everywhere.
4. **Placeholder scan:** no TBD/TODO; every step carries code; every test has its run command and expected red/green outcome.
5. **v1 → v2 review wave (2026-08-22):** two executed dry runs (core T0–T8+T16; ask T9–T18 incl. the real capture, $0.0655) went red→green with zero implementation edits and five test-side one-liners (now folded in: T8 sweep drain, T12 comment + `ROW_B`, T15 agent-finish accessor, T16 `'result/success'`); the Fable adjudication's seven MAJORs are folded in (get_run/list_runs redaction, `settle()` + post-finish semantics, the in-flight card-flip rule R-B, the spawned-env test, the `tool-deps.mjs` scan, the agent-finish frame order, the T15→T16 DAG edge). Mutation audits: 52/58 caught; the six misses now have assertions (T5 multi-key, T6 two PEM blocks, T9 origin/aliasing, T10 empty-text header, T12 25-row limit clamp, T17 replay text/cost).
6. **v2 → v3 (2026-08-22):** v2 was re-executed end to end by a zero-context agent (18 tasks, zero implementation edits, 3127/3127, fresh capture $0.078) and cold-reviewed by a second Fable pass. Folded in: rule R-C gains the abort branch (Stop/timeout are never retried and never `ask-error`ed as a turn failure); R-A persists the card mid-turn via `setMessageBlocks`; new rules R-F (mock markers mandatory + T16 step 3c hardening under `dontAsk`) and R-G (spawn wiring); P3 block shapes and fixture→frame derivation in the contract; the dated-model-key twin in `estimateAgentCosts`; per-string-value redaction in the fixture sanitiser; the capture script retries model flakiness (never recipe violations) and always cleans up; the T12 limit-clamp test now observes both bounds (130 rows); fourteen placeholder resolutions written into the steps; anchor note in Global Constraints.

## Execution notes

- Waves W1–W6 (see the DAG) may run in parallel inside a wave with one worktree per task; the `claude-runner.mjs` touch points belong to T1 and T16 only — T16 must rebase on T1 AND wait for T15 (its test imports the reducer).
- The only task that needs network/OAuth is T17 step 6 (`npm run ask:fixtures`, measured $0.0655; budget ≈ $0.15). Everything else is offline. The capture script reads the developer's real `~/.worca-cc/settings.json` ONLY through `resolveModelEnv` (read-only, for per-model routing env) — it never writes outside its temp base.
