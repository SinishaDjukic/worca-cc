...
- **Suite baseline at `dbb47f68`: 3230 pass / 0 fail / 0 skip (~71 s, node v25.6.1)** after `npm ci`. Per-task counts in this plan are v1 ESTIMATES (marked ≈); the refine/dry-run pass replaces them with measured values.
...
| # | fact | consequence |
|---|---|---|
| P3-F1 | P1/P2 changed nothing under `ui/public/` (`git diff --stat 79dc9256 dbb47f68 -- ui/public/` is empty). app.js = 14271 lines, index.html = 1381, style.css = 2820. | Spec §10.2 anchors hold with three cosmetic drifts: `if (!msg.runId) return;` is at `app.js:603` (not 604); the Running Escape handler is `:12483-12493`; the final reduced-motion `@media` is at `style.css:2804` (its section comment at :2799). |
...
- **V3** schedule-on-dirty: one armed `raf(flush)` at a time; `flush` re-arms itself only when the selection guard left dirt. Elapsed: one `win.setInterval(tickElapsed, 1000)` started on send / on subscribing to an in-flight thread, cleared on done/error/stop.
...
Expected: every test FAILS with `Cannot find module … ui/public/ask-model.mjs` (the helper resolves — it only touches P1 modules).
...
Expected: **18 pass / 0 fail**, clean exit (no `--test-force-exit` on green).

- [ ] **Step 1.6: Full suite**

Run: `npm test`
Expected: **≈3248 pass / 0 fail** (baseline 3230 + 18; the refine pass pins the exact number).
...
test('ask-markdown: foreign classes are stripped, language-*/hljs-* survive on the right tags', async () => {
  ...
  const p = out.querySelector('p:not(pre p)');
...
Expected: FAIL — `Cannot find module … ui/public/ask-markdown.mjs`.
...
export function createMarkdownRenderer({ doc, load, hljsLoader }) {
  ...
  function render(text) {
    ...
    let frag;
    try {
      frag = mods.purifier.sanitize(html, { ALLOWED_TAGS, ALLOWED_ATTR, ALLOWED_URI_REGEXP, RETURN_DOM_FRAGMENT: true });
    } catch {
      return { kind: 'plain' };
    }
...
Run: `npm test`
Expected: **≈3261 pass / 0 fail** (previous + 13).
...
export function makePanel(overrides = {}) {
  ...
  // Drain the manual raf queue (repeat until settled — a flush may re-arm once).
  const flush = () => { for (let i = 0; i < 5 && rafQueue.length; i++) rafQueue.splice(0).forEach((fn) => fn()); };
...
Expected: FAIL — `Cannot find module … ui/public/ask-panel.mjs`.
...
```js
  ...
  function updateMeters() { /* [T5 replaces] */ }
  ...
  function destroy() {
    ...
    st.destroyed = true;
    closePopover({ focusTrigger: false });
    if (st.elapsedTimer) { win.clearInterval(st.elapsedTimer); st.elapsedTimer = null; }
...
and insert `jumpToLatest` immediately BEFORE the `// ---- mount ---…` comment line:
...
- [ ] **Step 5.3: Implement.** In `ui/public/ask-panel.mjs`, replace the five stubs
...
with the streaming section (the `updateMeters` stub from Task 3 STAYS — Task 6 owns it):
...
```js
  ...
  function startElapsed() {
    st.elapsedStart = now();
    if (st.elapsedTimer) win.clearInterval(st.elapsedTimer);
    st.elapsedTimer = win.setInterval(() => scheduleFlush(), 1000);
  }
...
Then delete the now-duplicated old stub lines for `subscribe`/`resync` if the Edit left any behind (the replacement above must leave exactly ONE definition of each function in the file — verify with `grep -c "function subscribe" ui/public/ask-panel.mjs` → 1, same for `pushServerFrame`, `onHello`, `resync`, `flushExtra`, `updateSendStop`).
...
Expected: **32 pass / 0 fail** (12 new — the selection test may report 1 skip if jsdom cannot hold a non-collapsed selection; record which).

- [ ] **Step 5.5: Full suite**

Run: `npm test`
Expected: **≈3293 pass / 0 fail**.
...
```js
  ...
  function updateMeters() {
    ...
    el.meterCost.textContent = fmtUsd(totals.costUsd) || '$0.00';
  ...
  async function sendMessage() {
    ...
    try {
      ...
      if (!st.model.thread().title) el.title.textContent = text.slice(0, 80); // deterministic title has no frame
      el.input.value = '';
      st.pendingFiles = [];
      renderChips();
      subscribe(id);
...
Expected: **42 pass / 0 fail** (10 new; earlier suites stay green — Task 3's shell test still finds `textarea.ask-input`).

- [ ] **Step 6.5: Full suite**

Run: `npm test`
Expected: **≈3303 pass / 0 fail**.
...
**Interfaces:**
- Consumes: `GET /api/projects → {projects:[{name,path,exists}]}` · `GET /api/workflows` (array or `{workflows}`) · `GET /api/guardrails` (array or `{sets}`) · `GET /api/workspaces` (array or `{workspaces}`, rows carry `id,name,projectPaths,projectKeys`) · `GET /api/branches?projectDir=` (array or `{branches}`) — the card tolerates both envelope shapes; the dry-run pins the real ones. `POST /api/run` and `POST /api/ask/threads/:id/cards/:cardId` per the inlined contract.
...
function apiHandler(recorder = {}) {
  return (url, opts) => {
    ...
    if (url === '/api/guardrails') return { ok: true, status: 200, json: async () => ({ sets: [{ id: 'permissive', name: 'Permissive' }, { id: 'normal', name: 'Normal' }, { id: 'strict', name: 'Strict' }] }) };
...
Expected: **32 pass / 0 fail** (10 new; the render and stream suites make no card-shape assertions, so the stub→form change cannot red them).

- [ ] **Step 8.5: Full suite**

Run: `npm test`
Expected: **≈3323 pass / 0 fail**.
...
**(a)** Widen the hljs variable selector (`:1877`) — change ONLY the selector line:
...
**(c)** Add the reduced-motion arm INSIDE the existing final block. In the `@media (prefers-reduced-motion: reduce)` block at (former) `:2804`, add after the `.rd-sec-logs .log::after{animation:none;}` line:
...
Expected: all green — the mount performs no network, adds no markup to index.html, and `askPanel?.` guards every seam. Then `npm test` → **≈3342 pass / 0 fail**. If ANY previously-green ui-* suite reds here, the failure is almost certainly a boot fetch or an unguarded seam — fix the seam, never the fence.
...
- [ ] **Step 11.1: Write the failing tests** — `test/ui-ask-card.test.mjs`, full file. Boot preamble: copy Task 10's (`test/ui-ask-integration.test.mjs`) verbatim including `askArms`, plus these fetch arms: `/api/workflows` → `{workflows:[{id:'wf_default',name:'Default'},{id:'wf_review',name:'Review only'}]}`, `/api/guardrails` → `{sets:[{id:'permissive',name:'Permissive'},{id:'normal',name:'Normal'}]}`, `/api/workspaces` → `{workspaces:[{id:'wks-team-00000001',name:'team',projectPaths:['/repos/proj','/repos/lib'],projectKeys:['proj-00000001','lib-00000002']}]}`, `/api/branches` → `{branches:['main','dev']}`, and `POST /api/run` → recorded into `runBodies` then `200 {runId:'run-uuid-1'}`. Shared driver:
...
test('ui-ask-card: Open in New Pipeline prefills the project form with the source forced to prompt', async () => {
  ...
  assert.equal(doc.querySelector('.prompt-pane, #prompt-pane, [data-pane="prompt"]')?.classList.contains('hidden') ?? false, false, 'prompt source visible');
});
...
Adapt the `boot()` helper so `boot({runResponse})` threads the `/api/run` override and returns `{window, calls, recv, runBodies}`; the prompt-pane selector in test 4 must be replaced with the REAL pane element (`el.promptPane`'s id/class as found in `index.html` — resolve it while writing the file, spec seam anchor `syncSourceToggle` `app.js:4649`). These two dry-run-sensitive spots are the ONLY permitted adaptations; every assertion stays.
...
**(a)** Insert `applyAskPrefill` directly after `openNewPipeline` (Task 10's block):
...
async function applyAskPrefill() {
  ...
  if (p.target === 'workspace') {
    ...
    if (p.workspaceId && el.workspaceSelect) {
      ...
      el.workspaceSelect.dispatchEvent(new Event('change', { bubbles: true }));
    }
...
Run: `npm test`
Expected: **≈3347 pass / 0 fail** (fences incl. `ui-newpipeline-*` stay green — `applyAskPrefill` is a no-op when `newPipelinePrefill` is null).
...
(only the count and its message change; every per-tip assertion in the loop already covers the new three).
...
**(a) `ui/public/index.html`** — insert between the Budget card's closing `</section>` (`:1205`) and the Chat-notifications comment (`:1207`):
...
**(b) `ui/public/app.js`** — after the budget-card block (`saveBudget` + listeners end near `:7513`), add the mirrored ask block:
...
and inside `loadSettings()` (`:7229`), where the fetched settings body is painted (next to the existing `paintSettings(...)`/budget paint call — locate by reading the function), add:
...
- [ ] **Step 13.1** Full suite: `npm test` → expected **≈3353 pass / 0 fail / 0 skip** (baseline 3230 + ≈123 new across Tasks 1–12; the refine pass pins the exact ladder). Green runs exit on their own — if the process hangs, a panel timer leaked (`.unref()`/clearInterval audit, Global Constraints).
- [ ] **Step 13.2** Fence greps, all must hold:
  ...
  - `grep -n 'ask-' ui/public/app.js | grep -v 'askPanel\|askQuestions\|asksQuestions\|run-ask-banner\|sic-ask\|newPipelinePrefill\|applyAskPrefill\|askMaxTurns\|askMaxBudgetUsd\|askLimits\|askNoCap\|ask-panel'` → nothing unexpected
  - `grep -rn 'chat\|channel' ui/public/ask-panel.mjs ui/public/ask-model.mjs ui/public/ask-markdown.mjs` → zero hits (reserved words)
...
8. **Q: Are the per-task suite counts binding?** — A: No — v1 estimates (≈ marks). The refine/dry-run pass measures and pins them; the 3230/0 baseline IS binding.
...
11. **Q: What may the implementer adapt without a deviation record?** — A: Only the two spots Task 11 marks (the `boot({runResponse})` plumbing and the prompt-pane selector) plus mechanical Edit-anchor drift (line numbers moved by earlier tasks in the same file). Everything else — names, payloads, assertions, error strings — is exact; a mismatch means STOP and record a deviation, not silently adapt.
