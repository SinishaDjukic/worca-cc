...
**Revision:** v2 (2026-08-23) — v1 (2026-08-22) was written after a five-agent recon/adjudication wave (A1 `ui/server.mjs` territory, A2 as-built-vs-contract verification, A3 lifecycle precedents, A4 test conventions + baseline, A5 Fable architecture adjudication; vendor facts verified empirically at the exact pins), then EXECUTED end to end by two independent dry-run agents (D1: Tasks 1–4 + 23-mutation audit; D2: Tasks 1–8 + 16-mutation audit). Every implementation hunk applied verbatim except two server-side fixes now folded in (the artifacts import, the pipeline-row mapping); the dry-runs' test-side corrections (three T4 fixtures that raced `run()`'s fs awaits, the T6 job-frame filter and title asserts, the T7 workspace fixture and HITL auto-answer pump) and the mutation-audit hardening (two `ask-follow` tests, write-order and broadcast assertions, a new preflight-failure test) are all folded in below. v1 landed its predicted counts exactly at every task; v2's counts shift for the added tests only.
...
Expected: `package.json` `dependencies` gains `"dompurify": "3.4.14"` and `"marked": "18.0.10"` (exact, no caret); `package-lock.json` gains both entries with the integrities above. Nothing else changes in `dependencies`. npm prints "added 3 packages" — the third is `@types/trusted-types` (an OPTIONAL dep of dompurify) which lands in the lock only; that is expected.
...
after(async () => {
  if (srv) {
    await new Promise((r) => {
      srv.close(r);
      srv.closeAllConnections?.(); // a red WS test never reaches ws.close() — don't wedge the file
    });
  }
...
A-1: `lookupPipelineRow` and `findPipelineRowById` are **NOT yet imported** (they appear nowhere in `ui/server.mjs` at `1b02d87b`) — extend the EXISTING `src/core/artifacts.mjs` import statement (never add a second one) so its last specifiers read:
...
A-2: below the existing `src/core/settings.mjs` import block (~`:22-37`) add:
...
Run: `WORCA_HOME=/tmp/worca-p2-home node --disable-warning=ExperimentalWarning --test test/ask-api-messages.test.mjs`
Expected: PASS (9/9), ~8 s total (`MOCK_SLOW` paces ~2.1 s per slow test; the R-F test's 400 ms title settle is ample — the mock title call resolves in ~40 ms).

- [ ] **Step 5: Full suite**

Run: `npm test`
Expected: **3217 pass / 0 fail** (3208 + 9).
...
after(async () => {
  ...
  await rm(projectDir2, { recursive: true, force: true }).catch(() => {});
});
...
Run: `WORCA_HOME=/tmp/worca-p2-home node --disable-warning=ExperimentalWarning --test test/ask-api-cards.test.mjs`
Expected: PASS (8/8), ~10-15 s (dry-run-measured 9.9 s for the seven pre-preflight tests — the auto-answer pump takes each mock pipeline to `done` in ~0.5 s).

- [ ] **Step 5: Full suite**

Run: `npm test`
Expected: **3225 pass / 0 fail** (3217 + 8). `/api/run` fences: `workspaces-api`, `api-workflows`, `api-workflow-defaults`, `ui-server-stepper-seed`, `ui-server-question-replay` all untouched-green (the ask hunks are no-ops without the ask fields).
...
Expected: **3225 pass / 0 fail** both times (flake shield). If `test/api-sources.test.mjs` ENOTEMPTYs or the skills-gate timing flake fires, re-run the single file before investigating.
...
| `POST /api/workflows` validates agent keys (`validateWorkflow`) — a ghost-key workflow must be written via `writeWorkflow` (store stamps only) | **verified** (route source + `test/api-workflows.test.mjs:79`) |
...
4. **Counts:** 3166 → T1 3171 → T2 3173 → T3 3183 → T4 3198 → T5 3208 → T6 3217 → T7 3225. (v1's 3181/3196/3206/3215/3222 were landed exactly by both dry runs; v2 adds the two `ask-follow` audit tests and the T7 preflight test.)
...
15. **Q: Why is the preflight-failure test's workflow written via `writeWorkflow` instead of `POST /api/workflows`?** — A: The route validates agent keys against the live registry (`validateWorkflow`); only the store write path can produce a workflow whose key stops resolving — which is exactly the production scenario (agent removed / plugin disabled after creation) the preflight gate exists for.
