...
### 4.4b Sub-agent model policy (per fan-out node)

A model selection governs the node's own `claude -p`. It says nothing about the **Task/Agent children** that node spawns when `fanOut` is on. `subagentModel` — the fifth per-node tunable, beside `model` / `effort` / `fanOut` / `askQuestions` — is that policy: `sonnet` | `opus` | `fable` | `auto` | `inherit`, with **unset resolving to `auto`** (agents choose their children's models BY DEFAULT). `''` is the stored form of "unset" and the write APIs' explicit clear; `inherit` is a distinct stored value.

**Vocabulary** (`model-env.mjs`, beside `EFFORTS` and the reserved-key policy): deliberately NOT the worca catalog — the CLI's Task tool takes an **alias enum**, so a catalog id or an `ANTHROPIC_MODEL` wire id would be refused at spawn time. Haiku is excluded by product decision.

**One wire.** The CLI resolves a child's model as Task-call `model` > the agent definition's own `model:` frontmatter > `CLAUDE_CODE_SUBAGENT_MODEL` > parent. Every purpose-built agent the fan-out prompt steers toward carries frontmatter, so an env floor binds almost nothing — the drafted `#2` env wire was **removed** for exactly that reason, and `CLAUDE_CODE_SUBAGENT_MODEL` is now a **reserved model-env key** so a catalog entry cannot resurrect a silent floor either (a user-catalog write rejects; a plugin manifest carrying it is degraded — key stripped with a warning — not rejected, §9.1). What remains is the prompt block (`subagentModelDirective`, appended by `fanOutDirective`, gated on `ctxFanOut` so a non-fan-out prompt keeps today's bytes): both modes instruct passing an **explicit `model` on every Task/Agent call** — the only wire that outranks frontmatter — and **attribute the request to the operator** (the Task schema tells the model to set `model` only when a user asked for it, so an unattributed hint would correctly be ignored). `inherit` contributes nothing: children ride the CLI's own resolution, the pre-feature behavior.

`auto` = a choice rubric keyed on **who checks the output** (mechanical/verifiable → `sonnet`; codebase judgment the parent builds on → `opus`; a verdict nothing downstream re-checks → `fable`), never on apparent difficulty: the agent is sizing a sub-task before anything has been read, and that estimate runs optimistic. The tiers are phrased as read-only investigation, matching the fan-out block's "sub-agents never write" contract.

**Persistence** rides the existing layers unchanged: `project_config.steps[role]`, `config_workflow_nodes.subagent_model` (v25), a v2 template node's `config`, resolved by `resolveGraph` — each layer **enum-guarded** there, because the template layer has no write-time validation (a plugin import checks nothing), so an off-vocabulary value falls through instead of freezing into the manifest — and recorded RAW in the run manifest so a resume keeps it; the runtime (`ctxSubagentModel`) resolves unset → `auto`. Unlike `model`/`effort` the write APIs **preserve it when the field is absent** (`inheritOrSubagentModel`): the field arrived after those endpoints shipped, and an older client must not wipe a policy it does not know about. `''` is the explicit clear.

**Audit.** `sub_agents.run_model` (v25) records what each child ran on — the alias its Task call named (the directive asks for an explicit one on every call), else the parent's model, which is what an omitted param inherits. Known gap: a child spawned as a frontmatter-`model:` agent that ignored the directive records the parent's model (the frontmatter is invisible in the stream). This is deliberately NOT the in-memory pricing model. The run/history sub-agent rows paint it as a neutral pill; the `subagent` WS delta carries it so the Running view paints live; a pre-v25 row records nothing and paints none.

### 4.5 Validation

- `subagentModel` validates against the **alias enum**, not the catalog, via the shared `subagentModelIssue` (one message for `setStep`, `setNodeModel` and `nodeDefaultsError` → 400); `sanitizeNodeDefaults` drops an illegal value with a warn rather than storing it, and `resolveGraph` enum-guards every layer, so a value that escapes validation (plugin template import) falls through to the auto default instead of reaching the run.
...
Validation (`plugin-manifest.mjs`, install fails on error): model ids non-empty + case-insensitively unique within the plugin, efforts a subset of `EFFORTS`, every `{"secret"}` ref names a declared `modelSecrets` key, `modelSecrets` keys match the config-field `KEY_RE` and are unique, and `cost` passes `assertModelCost` — the SAME validator `settings.mjs` runs on a global entry, shared from the zero-import leaf `model-env.mjs` so neither catalog layer has to import the other. A **reserved env key** (`isReservedModelEnvKey`) **degrades instead of rejecting**: the key is stripped and a warning names it (error under `--strict`, like unknown fields) — a published manifest is a third-party artifact its installer cannot edit, and a host release that GROWS the reserved list (`CLAUDE_CODE_SUBAGENT_MODEL` did in §4.4b) must not retroactively brick installed/marketplace plugins; the spawn-time `prepareModelEnv` drop stays as the second gate either way. `POST /api/models` (the user's own catalog) keeps the hard write-time reject — that author can fix their input. Unknown fields warn (error under `--strict`), like everything else in the manifest.
